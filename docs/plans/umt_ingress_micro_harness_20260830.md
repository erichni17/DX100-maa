...
opcode-11 harness: D32/G16, D32/G31, D32/G32, and D64/G32. v6 hard-pins the
...
python3 tests/lanl_maa/umt_ingress_micro_harness.py freeze-contract \
  --campaign-root CAMPAIGN --output CAMPAIGN/ingress-contract-v6.json \
...
python3 tests/lanl_maa/umt_ingress_micro_harness.py dry-dispatch \
  --campaign-root CAMPAIGN --contract CAMPAIGN/ingress-contract-v6.json \
  --contract-sha256 CONTRACT_SHA \
  --output CAMPAIGN/identity/ingress-dry-dispatch-v6.json
```
...
freeze the v6 contract and only then record (not execute) its arm commands.
...
test driver's exact result marker. It also takes the frozen v6 contract and
