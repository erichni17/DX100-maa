...
"$iverilog" -B "$ivl_base" -g2012 -Wall \
    -s lanl_maa_operation_retirement_overlay_tb \
    -o "$build_root/rtl-smoke/lanl_maa_operation_retirement_overlay_tb" \
    "$harness/rtl/LanlMaaOperationRetirementOverlay.v" \
    "$harness/tests/lanl_maa_operation_retirement_overlay_tb.v"

"$vvp" -M "$ivl_base" \
    "$build_root/rtl-smoke/lanl_maa_operation_retirement_overlay_tb"

"$iverilog" -B "$ivl_base" -g2012 -Wall \
    -I "$hardfloat/RISCV" \
    -I "$hardfloat" \
    -s lanl_maa_fp64_retirement_overlay_tb \
    -o "$build_root/rtl-smoke/lanl_maa_fp64_retirement_overlay_tb" \
    ...
    "$harness/rtl/LanlFp64Completion2WSplit.v" \
    "$harness/rtl/LanlMaaOperationRetirementOverlay.v" \
    "$harness/tests/lanl_maa_fp64_retirement_overlay_tb.v"

exec "$vvp" -M "$ivl_base" \
    "$build_root/rtl-smoke/lanl_maa_fp64_retirement_overlay_tb"
