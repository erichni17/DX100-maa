...
"$iverilog" -B "$ivl_base" -g2012 \
    -DLANL_FP64_DUAL_SHARED_RECODE \
    -I "$hardfloat/RISCV" \
    -I "$hardfloat" \
    -s lanl_fp64_dual_portfolio_tb \
    -o "$build_root/rtl-smoke/lanl_fp64_dual_shared_recode_tb" \
    ...
    "$harness/tests/lanl_fp64_dual_portfolio_tb.v"

exec "$vvp" -M "$ivl_base" \
    "$build_root/rtl-smoke/lanl_fp64_dual_shared_recode_tb"
