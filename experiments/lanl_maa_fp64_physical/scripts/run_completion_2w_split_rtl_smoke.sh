...
"$iverilog" -B "$ivl_base" -g2012 -Wall \
    -DLANL_COMPLETION_SPLIT \
    -I "$hardfloat/RISCV" \
    -I "$hardfloat" \
    -s lanl_fp64_completion_2w_tb \
    -o "$build_root/rtl-smoke/lanl_fp64_completion_2w_split_tb" \
    ...
    "$harness/tests/lanl_fp64_completion_2w_tb.v"

exec "$vvp" -M "$ivl_base" \
    "$build_root/rtl-smoke/lanl_fp64_completion_2w_split_tb"
