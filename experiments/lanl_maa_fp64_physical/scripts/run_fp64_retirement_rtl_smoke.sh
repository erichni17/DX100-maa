...
"$iverilog" -B "$ivl_base" -g2012 -Wall \
    -s lanl_maa_operation_retirement_tb \
    -o "$build_root/rtl-smoke/lanl_maa_operation_retirement_tb" \
    "$harness/rtl/LanlMaaOperationRetirement.v" \
    "$harness/tests/lanl_maa_operation_retirement_tb.v"

"$vvp" -M "$ivl_base" \
    "$build_root/rtl-smoke/lanl_maa_operation_retirement_tb"

"$iverilog" -B "$ivl_base" -g2012 -Wall \
    -I "$hardfloat/RISCV" \
    -I "$hardfloat" \
    -s lanl_maa_fp64_retirement_tb \
    -o "$build_root/rtl-smoke/lanl_maa_fp64_retirement_tb" \
    ...
    "$harness/rtl/LanlMaaOperationRetirement.v" \
    "$harness/tests/lanl_maa_fp64_retirement_tb.v"

exec "$vvp" -M "$ivl_base" \
    "$build_root/rtl-smoke/lanl_maa_fp64_retirement_tb"
