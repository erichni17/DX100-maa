#!/usr/bin/env python3
"""Adversarial contract tests for the full cache-on direct4/q16 lane-4 run."""
...
RUNNER_PATH = (
    ROOT
    / "experiments/scripts/run_cg_direct4_product_page_fed_q16_lane4_full.py"
)
RUNNER_TEXT = RUNNER_PATH.read_text(encoding="utf-8")
SPEC = importlib.util.spec_from_file_location(
    "cg_direct4_lane4_full", RUNNER_PATH
)
...
def good_stats() -> dict[str, int]:
    return {
        ...
        "IND_SoaJitValueReadIssues": 11_266_321,
        "IND_SoaJitValueReadResponses": 11_266_321,
        "IND_SoaJitValueFills": 11_266_321,
        "IND_SoaJitValueCachedResponses": 11_266_321,
        "IND_SoaJitValueHits": 168_302_259,
        "IND_SoaJitValueMergedWaiters": 60,
        ...
        "IND_SoaJitPageFedAdmitCommands": runner.base.EXPECTED_PAGES,
        "IND_SoaJitPageFedCloseCommands": runner.EXPECTED_WINDOWS,
        "IND_SoaJitPageFedCommandResponses": (
            runner.base.EXPECTED_PAGES + runner.EXPECTED_WINDOWS
        ),
        ...
        "STR_PublishIssues": runner.base.EXPECTED_PUBLISH_LINES,
        "STR_PublishAccepts": runner.base.EXPECTED_PUBLISH_LINES,
        "STR_PublishWriteResponses": runner.base.EXPECTED_PUBLISH_LINES,
        "STR_PublishTerminals": runner.base.EXPECTED_PAGES,
        "IND_SoaJitValueEvictions": 10_915_601,
...
def test_resolved_config_requires_cache_on_owners_tiles_and_lane4() -> None:
...
def test_terminal_requires_direct4_p16_false_q16_true_and_exact_backing() -> (
    None
):
    fields = {
        "full_windows": str(runner.base.EXPECTED_WINDOWS),
        "staged_index_words": str(runner.base.EXPECTED_WORDS),
        "staged_value_words": "0",
        "product_words": str(runner.base.EXPECTED_WORDS),
        "index_publish_pages": "0",
        "value_publish_pages": "0",
        "product_publish_pages": str(runner.base.EXPECTED_PAGES),
        "logical_alu_vectors": "0",
        "physical_alu_vectors": str(runner.base.EXPECTED_PAGES),
        "logical_page_windows": "0",
        "physical_page_product_windows": "0",
        "page_fed_product_windows": "0",
        "direct4_product_page_fed_q16_windows": str(
            runner.base.EXPECTED_WINDOWS
        ),
        "virtual_p_gather_windows": "0",
        "physical_p_gather_pages": str(runner.base.EXPECTED_PAGES),
        "page_fed_admit_pages": str(runner.base.EXPECTED_PAGES),
        "page_fed_closes": str(runner.base.EXPECTED_WINDOWS),
        "q_spmv_eligible_windows": str(runner.base.EXPECTED_Q_WINDOWS),
        "q_spmv_routed_windows": str(runner.base.EXPECTED_Q_WINDOWS),
        "residual_spmv_eligible_windows": str(
            runner.base.EXPECTED_RESIDUAL_WINDOWS
        ),
    ...
    for key, bad in (
        ...
        changed[key] = bad
        with unittest.TestCase().assertRaises(RuntimeError):
            runner.base.validate_terminal(changed)


def test_selection_authority_is_frozen_lane4_exact_faster_evidence() -> None:
    identity = runner.validate_lane_selection_authority()
    assert identity["selected_lane"] == 4
    assert identity["decision"] == "ACCEPT_ALL_EXACT_FASTER_ARMS_CONFIRMED"
...
def test_accepted_lane1_is_unreadable_before_pass_and_exact_after_pass() -> (
    ...
    assert result["performance"]["candidate"] == 123_968_991_971
    assert stats["simTicks"] == 123_968_991_971
...
def test_comparison_requires_conserved_and_value_retention_identity() -> None:
    ...
    try:
        ...
        assert comparison["accepted_lane_1"] == 123_968_991_971
...
def test_zero_incremental_bytes_and_ports_for_fixed_four_owner_pool() -> None:
