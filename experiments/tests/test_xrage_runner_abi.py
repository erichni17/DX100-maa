...
class XrageRunnerAbiTest(unittest.TestCase):
    ...
    def test_complete_line_payload_width_is_explicit_and_recorded(self):
        ...
        self.assertIn("complete_line_payload_scheduled_words", script)
        self.assertIn("complete_line_payload_serial_read_cycles", script)
        self.assertIn("MAA_VIRTUAL_COMPLETE_LINE_PAYLOAD_BANKS", script)
        self.assertIn("complete_line_payload_bank_conflict_cycles", script)
    ...
    def test_combiner_banks_are_explicit_and_recorded(self):
        script = RUNNER.read_text(encoding="utf-8")
        self.assertIn("MAA_VIRTUAL_COMBINE_BANKS", script)
        self.assertIn("virtual_combine_banks=%s", script)
        self.assertIn('--maa_virtual_combine_banks="$combine_banks"', script)
        self.assertIn("combine_bank_accesses", script)
        self.assertIn("combine_bank_conflicts", script)
