...
def parse_arm(arm: Path, cg_na: int, lanes: int) -> dict[str, object]:
    ...
    stats = parsed["stats"]
    # The inherited parser closes the primary work ledger. Parse the remaining
    # physical counters explicitly; context stalls are reported but are not
    # conserved because apply-lane parallelism is expected to reduce them.
    for name in CONSERVE_STATS + ("IND_SoaJitContextStalls",):
