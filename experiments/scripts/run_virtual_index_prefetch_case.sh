...
/usr/bin/time -f 'checkpoint_wall=%e checkpoint_rss_kb=%M' \
    ...
    --max-checkpoints=1 --cmd "$binary" --options "$n random" \
...
/usr/bin/time -f 'restore_wall=%e restore_rss_kb=%M' \
    ...
    --cmd "$binary" --options "$n random" > "$out/restore.log" 2>&1
restore_rc=$?
set -e
...
result_count=$(grep -Ec \
    "^VIRTUAL_GATHER_RESULT n=${n} pattern=random hash=[0-9]+ errors=0$" \
...
output_hash=$(sed -nE \
    "s/^VIRTUAL_GATHER_RESULT n=${n} pattern=random hash=([0-9]+) errors=0$/\\1/p" \
    "$out/restore.log")
[[ $output_hash =~ ^[0-9]+$ ]] || {
...
{
    printf 'elements\tbuffer_lines\tbuffer_bytes\toutput_hash\tsimTicks\tsimInsts'
    printf '\tline_reads\tline_hwm\tindex_words\tindex_hwm\trt_full'
    printf '\twrite_issues\twrite_completions\tspd_read_cycles\n'
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$n" "$buffer_lines" "$((buffer_lines * 64))" "$output_hash" \
        "$ticks" "$insts" "$line_reads" "$line_hwm" "$index_words" \
        "$index_hwm" "$rt_full" "$write_issues" "$write_completions" \
        "$spd_reads"
} > "$out/result.tsv"
