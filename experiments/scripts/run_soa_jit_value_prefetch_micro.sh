...
# Exact full-16K/physical-4K SoA/JIT value-prefetch credit sweep.
if [[ $# -lt 1 || $# -gt 2 ]]; then
...
printf 'credits\tsimTicks\tvalue_reads\tprefetch_issues\tprefetch_promotions\tprefetch_discards\tprefetch_owned\tprefetch_credit_stalls\tprefetch_hwm\tvalue_stalls\tcontext_stalls\n' >"$out/results.tsv"
for credits in 0 1 2 4 8; do
    run="$out/p$credits"
    mkdir "$run"
    run_gem5 "$gem5" --listener-mode=off --outdir="$run" \
        --debug-flags=MAAVirtualTrace --debug-file=soa_jit_trace.log \
        "${common[@]}" --maa_soa_jit_active_value_owners=32 \
        --maa_soa_jit_value_prefetch_credits="$credits" \
        >"$run/restore.log" 2>&1
    ...
    grep -Fq 'soa_jit_active_value_owners=32' "$run/config.ini"
    grep -Fq "soa_jit_value_prefetch_credits=$credits" "$run/config.ini"
    ...
    pfi=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchIssues)
    pfr=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchResponses)
    pfp=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchPromotions)
    pfd=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchDiscards)
    pfo=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchOwned)
    pfs=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchCreditStalls)
    pfa=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchActiveCredits)
    pfh=$(stat_value "$stats" system.maa.I0_IND_SoaJitValuePrefetchHighWater)
    ...
    need_equal "$pfi" "$pfr" prefetch_response_ledger
    need_equal "$pfr" "$((pfp + pfd))" prefetch_terminal_ledger
    need_equal "$terminal" 2 terminal_ledger
    need_equal "$pfa" "$((credits * terminal))" active_credit_ledger
    [[ $pfh -le $((credits * terminal)) ]]
    ...
    active_prefetch=$(sed -n 's/.*active_prefetch_credits=\([0-9][0-9]*\).*/\1/p' <<<"$storage")
    ...
    [[ $cache_hwm -ge $terminal && $cache_hwm -le $((32 * terminal)) ]]
    need_equal "$active_prefetch" "$credits" storage_prefetch_credits
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$credits" "$ticks" "$vi" "$pfi" "$pfp" "$pfd" "$pfo" \
        "$pfs" "$pfh" "$value_stalls" "$context_stalls" \
        >>"$out/results.tsv"
done

{
    ...
    printf 'fixed_controls=contexts=32,predicate_credits=16,lookahead=8,index_lines=8,apply_lanes=1,value_owners=32\n'
    printf 'timeout_seconds=%s\n' "$timeout_seconds"
} >"$out/manifest.txt"

awk 'NR == 1 { next } $1 == 0 { base=$2 } NR > 1 { printf "credits=%s simTicks=%s speedup_vs_disabled=%.6fx value_reads=%s prefetch=%s promotions=%s discards=%s credit_stalls=%s value_stalls=%s\n", $1, $2, base/$2, $3, $4, $5, $6, $8, $10 }' \
    "$out/results.tsv" >"$out/summary.txt"
cat "$out/results.tsv"
cat "$out/summary.txt"
echo 'SOA_JIT_VALUE_PREFETCH_MICRO_PASS'
