...
binary="$out/bin/test_virtual_native_overlap"
"$cxx" -I"$root/benchmarks/API" -I"$root/include" \
    ...
    "$root/benchmarks/API/test_virtual_native_overlap.cpp" \
    -o "$binary"

{
    ...
    printf 'native_issue_delay_iterations=5000\n'
} > "$out/source.txt"
sha256sum "$gem5" \
...
git -C "$root" diff HEAD -- \
    ...
    benchmarks/API/test_virtual_native_overlap.cpp \
    experiments/scripts/validate_virtual_gather.sh \
    experiments/scripts/run_virtual_native_overlap.sh \
    > "$out/source.diff"

GEM5_BIN="$gem5" MAA_NUM_INDIRECT_UNITS_PER_MAA=2 \
    MAA_RETIREMENT_CACHE_RESPONSE_LATENCY=32768 \
    "$root/experiments/scripts/validate_virtual_gather.sh" \
    4096 native_after_virtual "$out/run" 21600 "$binary" \
...
read -r deferrals issues completions < <(
    awk '
        ...
        section == 1 && $1 ~ /^system\.maa\.I[01]_IND_VirtWriteIssues$/ {
            issues += $2
        }
        section == 1 &&
            $1 ~ /^system\.maa\.I[01]_IND_VirtWriteCompletions$/ {
            completions += $2
        }
...
if [[ $deferrals -le 0 || $issues -le 0 ||
      $issues -ne $completions ]]; then
    printf 'invalid overlap counters: deferrals=%s virtual=%s/%s\n' \
        "$deferrals" "$issues" "$completions" >&2
    exit 1
fi

printf 'VIRTUAL_NATIVE_OVERLAP_PASS deferrals=%s virtual=%s/%s\n' \
    "$deferrals" "$issues" "$completions"
: > "$out/native_overlap.pass"
