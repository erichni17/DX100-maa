...
DEFAULT_CERTIFICATE_ROOT = RUNS_ROOT / (
    "2026-08-26-cg-direct4-product-page-fed-q16-lane4-full-" "certificate-r2"
)
...
def validate_preflight(snapshot: dict[str, str]) -> dict[str, Any]:
    ...
    require(
        ...
        and status_before.startswith(
            "## codex/session-hybrid-q16-lane4-full-"
        ),
...
def validate_registered_process_absence(
    ...
    for pid in (REGISTERED_MAIN_PID, REGISTERED_WATCHER_PID):
        require(
            not (proc_root / str(pid)).exists(),
            f"registered PID {pid} is live",
        )
...
def validate_service_terminal(
    ...
    terminal = validate_service_properties(
        properties or query_service_properties()
    )
