...
sha256sum \
    ...
    "$root/src/mem/MAA/StreamAccess.hh" \
...
native_hash=$(awk -F '\t' 'NR == 2 { print $2 }' \
    "$out/native_direct_16k/result.tsv")
hybrid_hash=$(awk -F '\t' 'NR == 2 { print $2 }' \
    "$out/transparent_4k/result.tsv")
[[ -n $native_hash && $native_hash == "$hybrid_hash" ]]
[[ $(<"$out/native_direct_16k/restore.exit") == 0 ]]
