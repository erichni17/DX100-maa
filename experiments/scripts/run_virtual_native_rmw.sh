...
binary="$out/bin/test_virtual_native_rmw"
"$cxx" -I"$root/benchmarks/API" -I"$root/include" \
    ...
    "$root/benchmarks/API/test_virtual_native_rmw.cpp" \
    -o "$binary"
...
git -C "$root" diff HEAD -- \
    ...
    benchmarks/API/test_virtual_native_rmw.cpp \
    experiments/scripts/validate_virtual_gather.sh \
    experiments/scripts/run_virtual_native_rmw.sh \
    > "$out/source.diff"

GEM5_BIN="$gem5" MAA_NUM_INDIRECT_UNITS_PER_MAA=4 \
    MAA_RETIREMENT_CACHE_RESPONSE_LATENCY=32768 \
    "$root/experiments/scripts/validate_virtual_gather.sh" \
    4096 native_rmw_after_virtual "$out/run" 21600 "$binary" \
    384 96 64 4096 1 0 480 4 4 4

read -r deferrals issues completions < <(
    awk '
        ...
        section == 1 && $1 ~ /^system\.maa\.I[0-3]_IND_VirtWriteIssues$/ {
            issues += $2
        }
        section == 1 &&
            $1 ~ /^system\.maa\.I[0-3]_IND_VirtWriteCompletions$/ {
            completions += $2
        }
        /^---------- End Simulation Statistics/ && section == 1 {
            printf "%d %d %d\n", deferrals, issues, completions
            exit
        }
    ' "$out/run/stats.txt"
)

if [[ $deferrals -lt 1 || $issues -le 0 ||
      $issues -ne $completions ]]; then
    printf 'invalid RMW counters: deferrals=%s virtual=%s/%s\n' \
        "$deferrals" "$issues" "$completions" >&2
    exit 1
fi

printf 'VIRTUAL_NATIVE_RMW_PASS deferrals=%s virtual=%s/%s\n' \
    "$deferrals" "$issues" "$completions"
: > "$out/native_rmw.pass"
