...
UNIT=dx100-full-tile-t8-surge-recovery2-20260722
...
for unit in \
    ...
    dx100-full-tile-surge-recovery2-20260722.service; do
...
exec systemd-run --user --no-block --collect \
  --unit="$UNIT" \
  --description="DX100 OOM-contained 8K tile surge lane" \
  ...
  --workflow "$RUN_ROOT/recovery2-t8-surge-workflow.json" \
  --run-root "$RUN_ROOT" \
  "${allowed[@]}" \
  ...
  --artifact-stem recovery2-t8-surge-manager
