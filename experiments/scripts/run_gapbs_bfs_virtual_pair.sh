...
extract_first_stats() {
    awk '
        ...
        section == 1 && $1 == "system.maa.cpu_spd_data_read_retries" {
            retries=$2; retries_seen++
        }
        ...
        /^---------- End Simulation Statistics/ && section == 1 {
            if (ticks_seen != 1 || cycles_seen != 1 || indrd_seen != 1 ||
                retries_seen != 1 || issues_seen == 0 ||
                completions_seen == 0 || ticks !~ /^[0-9]+$/ ||
                cycles !~ /^[0-9]+$/ || indrd !~ /^[0-9]+$/ ||
                retries !~ /^[0-9]+$/)
                exit 2
            printf "%s\n%s\n%s\n%s\n%.0f\n%.0f\n", ticks, cycles,
                indrd, retries, issues, completions
            emitted=1
            exit 0
        }
...
restore_one() {
    ...
    local stats_blob ticks=NA cycles=NA indrd=NA retries=NA
    ...
    if stats_blob=$(extract_first_stats "$out/stats.txt"); then
        ...
        if [[ ${#stats_fields[@]} -eq 6 ]]; then
            ticks=${stats_fields[0]}
            cycles=${stats_fields[1]}
            indrd=${stats_fields[2]}
            retries=${stats_fields[3]}
            issues=${stats_fields[4]}
            completions=${stats_fields[5]}
        else
            valid=0
        fi
    else
