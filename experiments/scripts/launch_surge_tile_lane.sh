...
UNIT=dx100-full-tile-surge-recovery2-20260722
...
[[ -n "$NORMAL_CGROUP_REL" && -n "$GATE_CGROUP_REL" && -n "$AUXILIARY_CGROUP_REL" ]]
...
exec systemd-run --user --no-block --collect \
  --unit="$UNIT" \
  --description="DX100 OOM-contained XRAGE surge tile lane" \
  ...
  --workflow "$RUN_ROOT/recovery2-surge-workflow.json" \
  --run-root "$RUN_ROOT" \
  ...
  --artifact-stem recovery2-surge-manager
