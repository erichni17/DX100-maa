...
UNIT=dx100-full-tile-xrage64-recovery2-20260722
allowed=()
for unit in \
...
if ((${#allowed[@]} == 0)); then
  printf 'refusing XRAGE 64K launch without any owned live cgroup\n' >&2
  exit 1
fi
...
exec systemd-run --user --no-block --collect \
  ...
  --workflow "$RUN_ROOT/recovery2-xrage64-workflow.json" \
  --run-root "$RUN_ROOT" \
  "${allowed[@]}" \
