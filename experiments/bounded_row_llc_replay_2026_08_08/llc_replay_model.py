...
GENERAL_SUBRUN_UPPER_BOUND = PARTITIONS * math.ceil(
    FINITE_OFFSETS / FINITE_ROWS_PER_SLICE
)
    ...
    with path.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            if (
                ...
                raise ModelError(
                    f"wrong physical schema/event at line {line_number}"
                )
            record = Record(
                ...
                a_line_paddr=_strict_int(
                    raw.get("a_line_paddr"), "a_line_paddr"
                ),
                native_slice=_strict_int(
                    raw.get("native_slice"), "native_slice"
                ),
            ...
            if not 0 <= record.b_value < SOURCE_ELEMENTS:
                raise ModelError(
                    f"B[{record.itr}] is outside the frozen source"
                )
            if record.b_paddr % 4 or record.a_line_paddr % LINE_BYTES:
                raise ModelError(
                    f"unaligned physical record at itr {record.itr}"
                )
            ...
            if record.row != record.grow or not 0 <= record.wid < 8:
                raise ModelError(
                    f"decoded row/grow/wid mismatch at itr {record.itr}"
                )
            records.append(record)
    if len(records) != LOGICAL_ELEMENTS:
        raise ModelError(
            f"expected {LOGICAL_ELEMENTS} records, got {len(records)}"
        )
    return records
...
class FiniteEpoch:
    ...
    def can_insert(self, record: Record) -> tuple[bool, str | None]:
        ...
        for slot in slots:
            if (
                slot.grow == record.grow
                and len(slot.lines) < LINES_PER_ROW_SLOT
            ):
                return True, None
    ...
    def row_groups(self) -> int:
        return len(
            {(record.native_slice, record.grow) for record in self.records}
        )
...
def build_schedule(
    streams: Iterable[Sequence[Record]],
    offset_capacity: int,
    rows_per_slice: int,
) -> Schedule:
        ...
        for record in stream:
            ...
            if not accepted:
                if not epoch.records or reason is None:
                    raise ModelError(
                        "record cannot fit in an empty finite epoch"
                    )
                epochs.append(epoch)
                drains[reason] += 1
                epoch = FiniteEpoch(offset_capacity, rows_per_slice)
                ...
                if not accepted:
                    raise ModelError(
                        f"record remains illegal after drain: {reason}"
                    )
            epoch.insert(record)
    ...
    if len(set(expected_itrs)) != len(expected_itrs):
        raise ModelError(
            "schedule input contains duplicate logical iterations"
        )
    if sorted(placements) != sorted(expected_itrs):
        raise ModelError(
            "schedule does not place every logical iteration exactly once"
        )
    ...
    return Schedule(
        tuple(epochs), drains, issue_digest.hexdigest(), placement_digest
    )
...
def build_descriptor_runs(
    ...
    if run_records <= 0 or len(records) % run_records:
        raise ModelError(
            "descriptor runs must exactly cover the logical input"
        )
    runs = tuple(
        tuple(
            sorted(
                records[start : start + run_records], key=descriptor_sort_key
            )
        )
        for start in range(0, len(records), run_records)
    )
    if len(runs) != PARTITIONS or any(
        len(run) > FINITE_OFFSETS for run in runs
    ):
...
def finite_descriptor_subruns(
    ...
    if sum(len(run) for run in runs) != len(records):
        raise ModelError(
            "finite descriptor runs do not exactly cover the input"
        )
...
def nominal_four_run_geometry(records: Sequence[Record]) -> dict[str, object]:
    ...
    for chunk_number, start in enumerate(
        range(0, len(records), FINITE_OFFSETS)
    ):
        chunk = records[start : start + FINITE_OFFSETS]
        line_sets: dict[tuple[int, int], set[int]] = {}
        for record in chunk:
            line_sets.setdefault(
                (record.native_slice, record.grow), set()
            ).add(record.a_line_paddr)
        rows_by_slice = []
...
def merge_descriptor_runs(
    ...
    if sorted(record.itr for record in merged) != list(range(len(merged))):
        raise ModelError(
            "descriptor merge lost or duplicated result iterations"
        )
    return tuple(merged)
...
def _stage_budget_comparison(
    ...
    return {
        ...
        "analytical_headroom_vs_aug3_modulo_cycles": baseline_cycles
        - total_cycles,
...
def descriptor_spool_model(records: Sequence[Record]) -> dict[str, object]:
    ...
    for record in merged:
        ...
        line_fanout[record.a_line_paddr] = (
            line_fanout.get(record.a_line_paddr, 0) + 1
        )
    ...
    required_rate = next(
        rate
        for rate in (1, 2, 4)
        if sensitivity[str(rate)]["request_cases"][
            "aug3_full_control_request"
    ...
    return {
        ...
        "merge": {
            ...
            "dram_row_groups": len(
                {(record.native_slice, record.grow) for record in merged}
            ),
        ...
        "traffic": {
            ...
            "descriptor_padding_bytes": descriptor_backing_bytes
            - descriptor_bytes,
            ...
            "coherent_line_transfers": input_line_count(records)
            + 3 * descriptor_lines,
...
def replay_traffic(
    ...
    if record_bytes not in (4, 8):
        raise ModelError(
            "only the audited packed32 and generic64 formats exist"
        )
    if record_bytes == 4:
...
def evaluate(
    records: Sequence[Record], trace_sha256: str
) -> dict[str, object]:
    ...
    if modulo.placement_sha256 != replay.placement_sha256:
        raise ModelError(
            "stable replay changed modulo logical placement order"
        )
    ...
    replay_selector_cycles = math.ceil(
        replay_selector_records / FILTER_WORDS_PER_CYCLE
    )
