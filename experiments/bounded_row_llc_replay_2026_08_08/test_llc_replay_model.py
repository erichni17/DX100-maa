...
class LlcReplayModelTest(unittest.TestCase):
    ...
    def test_partition_overflow_forces_extra_finite_epoch(self) -> None:
        ...
        self.assertEqual(
            [len(epoch.records) for epoch in schedule.epochs], [16, 1]
        )
    ...
    def test_row_slot_overflow_drains_and_retries(self) -> None:
        ...
        self.assertEqual(
            [len(epoch.records) for epoch in schedule.epochs], [4, 1]
        )
    ...
    def test_generic_and_packed_replay_charge_coherent_writeback(self) -> None:
        ...
        self.assertGreater(
            generic["coherent_line_transfers"],
            packed["coherent_line_transfers"],
        )
    ...
    def test_four_head_merge_preserves_every_itr_and_wid(self) -> None:
        ...
        self.assertEqual(
            list(merged), sorted(merged, key=MODEL.descriptor_sort_key)
        )

    def test_descriptor_spool_is_finite_and_has_analytical_headroom(
        self,
    ) -> None:
        records = [
            record(
        ...
        self.assertFalse(
            spool["result_iteration_mapping"]["unbounded_mapping_state"]
        )
        self.assertEqual(
            spool[
        ...
        self.assertGreaterEqual(
            conservative["analytical_headroom_vs_aug3_modulo_stage_sum_pct"],
    ...
    def test_frozen_trace_result_has_eight_subruns_and_no_latency_claim(
        self,
    ) -> None:
    ...
    def test_trace_control_and_timing_calibration_are_not_mixed(self) -> None:
        ...
        self.assertNotEqual(
            trace["campaign_root"], calibration["campaign_root"]
        )
        self.assertEqual(
            (
                trace["simTicks"],
                trace["row_table_full_events"],
                trace["virtual_build_rounds"],
                trace["fill_cycles"],
        ...
        self.assertEqual(
            (
                full["simTicks"],
                full["row_table_full_events"],
                full["virtual_build_rounds"],
                full["fill_cycles"],
                full["request_cycles"],
            ),
        ...
        self.assertEqual(
            manifest["matched_timing_calibration_aug3"]["full_control"][
                "result_tsv_sha256"
            ],
