...
void ConfigurationBase::report() {
    ...
    if (kernel.compare("sg") == 0)
        total_bytes_moved = nruns *
                            (pattern_scatter.size() + pattern_gather.size()) * count *
                            sizeof(size_t);
...
void ConfigurationBase::setup() {
    ...
    if (kernel.compare("sg") == 0) {
        ...
        // #ifdef USE_OPENMP
        //     if (kernel.compare("gather") == 0 || kernel.compare("multigather") ==
        //     0) {
...
Configuration<Spatter::Serial>::Configuration(const size_t id,
    ...
    for (size_t i = 0; i < pattern.size(); ++i)
        pattern_int[i] = static_cast<int>(pattern[i]);
    pattern_gather_int.resize(pattern_gather.size());
    for (size_t i = 0; i < pattern_gather.size(); ++i)
        pattern_gather_int[i] = static_cast<int>(pattern_gather[i]);
        ...
        for (int j = 0; j < pattern_length; j += maa_tile_size) {
            ...
            if (maa_arm == "direct4fusedprefetch") {
                maa_indirect_load_virtual_index_prefetch<double>(
                    ...
                    tile1, dense.data() + j, reg1, reg2, reg3);
            } else if (maa_arm == "direct4prefetch") {
...
#endif
#ifdef MAA
    std::cout << "MAA scatter execution " << pattern_length << "/" << TILE_SIZE << std::endl;
#endif
    if (timed)
        timer.start();
#ifdef MAA
...
#pragma omp for
        for (int j = 0; j < pattern_length; j += TILE_SIZE) {
            maa_const(j, reg1);
            maa_stream_load<int>(pattern_int.data(), reg1, reg2, reg3, tile1);
            maa_stream_load<double>(dense.data(), reg1, reg2, reg3, tile2);
            maa_indirect_store_vector<double>(sparse.data(), tile1, tile2);
        }
        wait_ready(tile2);
#else
#pragma omp for
        for (int j = 0; j < pattern_length; ++j) {
            sparse[pattern[j]] = dense[j];
        }
#endif
    }
...
Configuration<Spatter::OpenMP>::Configuration(const size_t id,
                                              ...
                                              const aligned_vector<size_t> &pattern,
                                              const aligned_vector<size_t> &pattern_gather,
                                              aligned_vector<size_t> &pattern_scatter, aligned_vector<double> &sparse,
...
#pragma omp parallel for simd
  for (size_t i = 0; i < count; ++i)
    for (size_t j = 0; j < pattern_length; ++j)
      sparse[pattern[j] + delta * i] = dense[j + pattern_length * (i %
wrap)];
      */
