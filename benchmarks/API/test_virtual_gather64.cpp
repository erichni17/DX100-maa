...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < n; ++i) {
        if (pattern == "allfalse")
            ...
            conditions[i] = i < 2 || i >= n - 2 || i == 7 || i == 8;
        else
            conditions[i] = (i % 3) != 0;
        if (pattern == "fanout")
            indices[i] = 13;
    ...
    int64_t *result = pattern == "native"
                          ...
                          : backing;
    for (int i = 0; i < n; ++i) {
        const int64_t expected = conditioned && !conditions[i]
                                     ? -1
                                     : source[indices[i]];
