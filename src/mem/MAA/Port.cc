#include <algorithm>
#include <cassert>
#include <cstdint>
#include <string>

#include "base/addr_range.hh"
#include "base/logging.hh"
#include "base/trace.hh"
#include "debug/MAACachePort.hh"
#include "debug/MAAController.hh"
#include "debug/MAACpuPort.hh"
#include "debug/MAAMemPort.hh"
#include "debug/MAAPort.hh"
#include "mem/MAA/ALU.hh"
#include "mem/MAA/IF.hh"
#include "mem/MAA/IndirectAccess.hh"
#include "mem/MAA/Invalidator.hh"
#include "mem/MAA/MAA.hh"
#include "mem/MAA/RangeFuser.hh"
#include "mem/MAA/SPD.hh"
#include "mem/MAA/StreamAccess.hh"
#include "mem/packet.hh"
#include "params/MAA.hh"
#include "sim/cur_tick.hh"

#ifndef TRACING_ON
#define TRACING_ON 1
#endif
namespace gem5 {
namespace {

LogicalStreamResponseKind
logicalStreamRequestKind(const MemCmd &command)
{
    panic_if(command != MemCmd::ReadReq && command != MemCmd::ReadExReq &&
                 command != MemCmd::WriteReq,
             "%s: command %s has no logical stream counter ownership\n",
             __func__, command.toString());
    if (command == MemCmd::ReadReq)
        return LogicalStreamResponseKind::Read;
    if (command == MemCmd::ReadExReq)
        return LogicalStreamResponseKind::ReadEx;
    return LogicalStreamResponseKind::Write;
}

bool
decodeLogicalStreamResponseKind(const MemCmd &command,
                                LogicalStreamResponseKind &kind)
{
    if (command == MemCmd::ReadResp) {
        kind = LogicalStreamResponseKind::Read;
        return true;
    }
    if (command == MemCmd::ReadExResp) {
        kind = LogicalStreamResponseKind::ReadEx;
        return true;
    }
    if (command == MemCmd::WriteResp) {
        kind = LogicalStreamResponseKind::Write;
        return true;
    }
    return false;
}

bool
senderStateChainExcludes(const Packet::SenderState *state,
                         const Packet::SenderState *target)
{
    uint32_t depth = 0;
    while (state != nullptr && depth < MaxResponseSenderStateDepth) {
        if (state == target)
            return false;
        state = state->predecessor;
        ++depth;
    }
    // A longer or cyclic chain cannot prove unique ownership.
    return state == nullptr;
}

struct LogicalSenderStateSearch
{
    LogicalSPDTransactionState *state = nullptr;
    bool complete = false;
};

LogicalSenderStateSearch
findLogicalSenderStateBounded(Packet::SenderState *state)
{
    uint32_t depth = 0;
    while (state != nullptr && depth < MaxResponseSenderStateDepth) {
        if (auto *logical =
                dynamic_cast<LogicalSPDTransactionState *>(state)) {
            return {logical, true};
        }
        state = state->predecessor;
        ++depth;
    }
    return {nullptr, state == nullptr};
}

void
applyLogicalStreamCounterEvent(uint32_t &counter, const MemCmd &command,
                               LogicalStreamCounterEvent event, Addr address)
{
    const LogicalStreamCounterDecision decision =
        decideLogicalStreamCounterUpdate(logicalStreamRequestKind(command),
                                         event, counter);
    panic_if(!decision.valid,
             "%s: logical stream counter boundary violation for %s event %d "
             "at 0x%lx (count %u)\n",
             __func__, command.toString(), static_cast<int>(event), address,
             counter);
    counter = decision.value;
}

void
settleAcceptedStreamReadCounter(uint32_t &counter, bool logical,
                                const MemCmd &command, Addr address)
{
    if (logical) {
        applyLogicalStreamCounterEvent(
            counter, command, LogicalStreamCounterEvent::SendAccepted,
            address);
    } else {
        --counter;
    }
}

} // anonymous namespace

MAA::SenderStateOwnership
MAA::inspectSenderStateChain(Packet::SenderState *state) const
{
    SenderStateOwnership ownership;
    while (state != nullptr && ownership.depth < MaxResponseSenderStateDepth) {
        const auto begin = ownership.nodes.begin();
        const auto end = begin + ownership.depth;
        if (std::find(begin, end, state) != end)
            return ownership;
        ownership.nodes[ownership.depth] = state;
        if (dynamic_cast<LogicalSPDTransactionState *>(state) != nullptr) {
            ++ownership.logicalNodes;
            if (ownership.depth == 0)
                ownership.logicalAtTop = true;
        }
        ++ownership.depth;
        state = state->predecessor;
    }
    ownership.boundedAcyclic = state == nullptr;
    return ownership;
}

bool
MAA::senderStateMatchesSnapshot(const SenderStateOwnership &expected,
                                const SenderStateOwnership &received) const
{
    if (!expected.boundedAcyclic || !received.boundedAcyclic ||
        expected.depth > MaxResponseSenderStateDepth ||
        received.depth > MaxResponseSenderStateDepth ||
        expected.depth != received.depth ||
        expected.logicalNodes != received.logicalNodes ||
        expected.logicalAtTop != received.logicalAtTop) {
        return false;
    }
    return std::equal(expected.nodes.begin(),
                      expected.nodes.begin() + expected.depth,
                      received.nodes.begin());
}

bool
MAA::senderStateAliasedByOtherPacket(
    PacketPtr packet, const SenderStateOwnership &ownership) const
{
    const auto aliasesOwnership = [&](
        PacketPtr candidate, const SenderStateOwnership *snapshot) {
        if (candidate == nullptr || candidate == packet)
            return false;
        if (snapshot != nullptr &&
            snapshot->depth > MaxResponseSenderStateDepth) {
            return true;
        }
        for (std::size_t index = 0; index < ownership.depth; ++index) {
            if (snapshot != nullptr &&
                std::find(snapshot->nodes.begin(),
                          snapshot->nodes.begin() + snapshot->depth,
                          ownership.nodes[index]) !=
                    snapshot->nodes.begin() + snapshot->depth) {
                return true;
            }
            if (!senderStateChainExcludes(candidate->senderState,
                                          ownership.nodes[index])) {
                return true;
            }
        }
        return false;
    };
    for (const auto &entry : my_outstanding_pkt_map) {
        if (aliasesOwnership(entry.second.packet,
                             &entry.second.senderStateOwnership))
            return true;
    }
    for (const auto &address : my_deferred_pkt_map) {
        for (const DeferredPacket &entry : address.second) {
            if (aliasesOwnership(entry.packet, &entry.senderStateOwnership))
                return true;
        }
    }
    const auto queueAliases = [&](const auto *queues, unsigned count) {
        for (unsigned queue = 0; queue < count; ++queue) {
            for (const OutstandingPacket &entry : queues[queue]) {
                if (aliasesOwnership(entry.packet,
                                     &entry.senderStateOwnership))
                    return true;
            }
        }
        return false;
    };
    return queueAliases(my_outstanding_indirect_cache_read_pkts, num_cores) ||
           queueAliases(my_outstanding_indirect_cache_write_pkts, num_cores) ||
           queueAliases(my_outstanding_stream_cache_read_pkts, num_cores) ||
           queueAliases(my_outstanding_stream_cache_write_pkts, num_cores) ||
           queueAliases(my_outstanding_stream_mem_read_pkts, num_cores) ||
           queueAliases(my_outstanding_stream_mem_write_pkts, num_cores) ||
           queueAliases(my_outstanding_indirect_mem_read_pkts, num_channels) ||
           queueAliases(my_outstanding_indirect_mem_write_pkts, num_channels);
}

MAA::LogicalPacketProvenance
MAA::findLogicalPacketProvenance(PacketPtr packet) const
{
    LogicalPacketProvenance provenance;
    const auto record = [&](const LogicalStreamTransactionTag &transaction,
                            Addr address, const MemCmd &command,
                            bool deferred) {
        const bool had_candidate = provenance.deferredAliases != 0 ||
                                   provenance.sendAliases != 0;
        if (deferred)
            ++provenance.deferredAliases;
        else
            ++provenance.sendAliases;
        if (!had_candidate) {
            provenance.transaction = transaction;
            provenance.address = address;
            provenance.command = command;
            provenance.counterOwned = !deferred;
            return;
        }
        const bool counter_owned = !deferred;
        if (provenance.transaction != transaction ||
            provenance.address != address || provenance.command != command ||
            provenance.counterOwned != counter_owned) {
            provenance.ambiguous = true;
        }
    };

    for (const auto &address : my_deferred_pkt_map) {
        for (const DeferredPacket &entry : address.second) {
            if (entry.packet == packet && entry.logicalResponseManaged) {
                record(entry.logicalTransaction, entry.paddr, entry.cmd, true);
            }
        }
    }
    const auto scan_queue = [&](const auto *queues, unsigned count) {
        for (unsigned queue = 0; queue < count; ++queue) {
            for (const OutstandingPacket &entry : queues[queue]) {
                if (entry.packet == packet && entry.logicalResponseManaged) {
                    record(entry.logicalTransaction, entry.paddr, entry.cmd,
                           false);
                }
            }
        }
    };
    scan_queue(my_outstanding_indirect_cache_read_pkts, num_cores);
    scan_queue(my_outstanding_indirect_cache_write_pkts, num_cores);
    scan_queue(my_outstanding_stream_cache_read_pkts, num_cores);
    scan_queue(my_outstanding_stream_cache_write_pkts, num_cores);
    scan_queue(my_outstanding_stream_mem_read_pkts, num_cores);
    scan_queue(my_outstanding_stream_mem_write_pkts, num_cores);
    scan_queue(my_outstanding_indirect_mem_read_pkts, num_channels);
    scan_queue(my_outstanding_indirect_mem_write_pkts, num_channels);
    if (provenance.deferredAliases != 0 && provenance.sendAliases != 0)
        provenance.ambiguous = true;
    return provenance;
}

MAA::LogicalLedgerOwner
MAA::findUniqueLogicalLedgerOwner(
    const LogicalStreamTransactionTag &transaction, Addr address) const
{
    LogicalLedgerOwner owner;
    if (!transaction.valid())
        return owner;
    for (unsigned maa_id = 0; maa_id < num_maas; ++maa_id) {
        StreamAccessUnit *const stream = &streamAccessUnits[maa_id];
        if (!stream->ownsLogicalResponse(transaction, address))
            continue;
        if (owner.stream != nullptr)
            return {};
        owner = {stream, static_cast<int>(maa_id)};
    }
    return owner;
}

ResponseTeardownShape
MAA::responseTeardownShape() const
{
    ResponseTeardownShape shape;
    shape.mapOwners = my_outstanding_pkt_map.size();
    for (const auto &address : my_deferred_pkt_map)
        shape.deferredOwners += address.second.size();
    const auto count_queue = [](const auto *queues, unsigned count) {
        std::size_t entries = 0;
        for (unsigned queue = 0; queue < count; ++queue)
            entries += queues[queue].size();
        return entries;
    };
    shape.cacheSendOwners =
        count_queue(my_outstanding_indirect_cache_read_pkts, num_cores) +
        count_queue(my_outstanding_indirect_cache_write_pkts, num_cores) +
        count_queue(my_outstanding_stream_cache_read_pkts, num_cores) +
        count_queue(my_outstanding_stream_cache_write_pkts, num_cores) +
        count_queue(my_outstanding_stream_mem_read_pkts, num_cores) +
        count_queue(my_outstanding_stream_mem_write_pkts, num_cores);
    shape.memorySendOwners =
        count_queue(my_outstanding_indirect_mem_read_pkts, num_channels) +
        count_queue(my_outstanding_indirect_mem_write_pkts, num_channels);
    for (unsigned maa_id = 0; maa_id < num_maas; ++maa_id) {
        if (streamAccessUnits[maa_id].hasActiveLogicalResponse())
            ++shape.activeLogicalLedgers;
        shape.outstandingCounters += my_num_outstanding_stream_pkts[maa_id];
    }
    for (unsigned unit = 0; unit < num_indirect_units_total; ++unit)
        shape.outstandingCounters += my_num_outstanding_indirect_pkts[unit];
    shape.logicalOwnerRecords = my_logical_packet_owners.size();
    for (const CacheSidePort *port : cacheSidePorts)
        shape.cacheResponseCredits += port->responseCredits();
    for (const CacheSidePort *port : retirementSidePorts)
        shape.cacheResponseCredits += port->responseCredits();
    shape.pendingPostDeleteCompletion =
        afterDeleteCompletion.kind != AfterDeleteCompletionKind::None;
    return shape;
}

MAA::PacketAliasCounts
MAA::countPacketAliases(PacketPtr packet) const
{
    PacketAliasCounts aliases;
    for (const auto &entry : my_outstanding_pkt_map) {
        if (entry.second.packet == packet)
            ++aliases.outstanding;
    }
    for (const auto &address : my_deferred_pkt_map) {
        for (const DeferredPacket &entry : address.second) {
            if (entry.packet == packet)
                ++aliases.deferred;
        }
    }
    const auto countQueue = [packet](const auto *queues, unsigned count) {
        std::size_t aliases = 0;
        for (unsigned queue = 0; queue < count; ++queue) {
            for (const OutstandingPacket &entry : queues[queue]) {
                if (entry.packet == packet)
                    ++aliases;
            }
        }
        return aliases;
    };
    aliases.cacheSend =
        countQueue(my_outstanding_indirect_cache_read_pkts, num_cores) +
        countQueue(my_outstanding_indirect_cache_write_pkts, num_cores) +
        countQueue(my_outstanding_stream_cache_read_pkts, num_cores) +
        countQueue(my_outstanding_stream_cache_write_pkts, num_cores) +
        countQueue(my_outstanding_stream_mem_read_pkts, num_cores) +
        countQueue(my_outstanding_stream_mem_write_pkts, num_cores);
    aliases.memorySend =
        countQueue(my_outstanding_indirect_mem_read_pkts, num_channels) +
        countQueue(my_outstanding_indirect_mem_write_pkts, num_channels);
    return aliases;
}

MAA::PacketAliasCounts
MAA::erasePacketAliases(PacketPtr packet)
{
    const PacketAliasCounts aliases = countPacketAliases(packet);
    for (auto entry = my_outstanding_pkt_map.begin();
         entry != my_outstanding_pkt_map.end();) {
        if (entry->second.packet == packet)
            entry = my_outstanding_pkt_map.erase(entry);
        else
            ++entry;
    }
    for (auto address = my_deferred_pkt_map.begin();
         address != my_deferred_pkt_map.end();) {
        auto &queue = address->second;
        for (auto entry = queue.begin(); entry != queue.end();) {
            if (entry->packet == packet)
                entry = queue.erase(entry);
            else
                ++entry;
        }
        if (queue.empty())
            address = my_deferred_pkt_map.erase(address);
        else
            ++address;
    }
    const auto eraseQueue = [packet](auto *queues, unsigned count) {
        for (unsigned queue = 0; queue < count; ++queue) {
            for (auto entry = queues[queue].begin();
                 entry != queues[queue].end();) {
                if (entry->packet == packet)
                    entry = queues[queue].erase(entry);
                else
                    ++entry;
            }
        }
    };
    eraseQueue(my_outstanding_indirect_cache_read_pkts, num_cores);
    eraseQueue(my_outstanding_indirect_cache_write_pkts, num_cores);
    eraseQueue(my_outstanding_stream_cache_read_pkts, num_cores);
    eraseQueue(my_outstanding_stream_cache_write_pkts, num_cores);
    eraseQueue(my_outstanding_stream_mem_read_pkts, num_cores);
    eraseQueue(my_outstanding_stream_mem_write_pkts, num_cores);
    eraseQueue(my_outstanding_indirect_mem_read_pkts, num_channels);
    eraseQueue(my_outstanding_indirect_mem_write_pkts, num_channels);

    // sendCacheEvent/sendMemEvent and the blocked retry flags carry no
    // PacketPtr; they rescan the queues above.  The RequestPort packet queues
    // are likewise unused because these paths call sendTimingReq directly.
    return aliases;
}

void
MAA::releaseRecordedLogicalSenderState(
    PacketPtr packet, const LogicalPacketOwner &owner)
{
    if (owner.senderStateOwnership.depth == 0)
        return;
    Packet::SenderState *const recorded =
        owner.senderStateOwnership.nodes[0];
    if (recorded == nullptr)
        return;
    const auto detach = [recorded](PacketPtr candidate) {
        if (candidate == nullptr)
            return;
        if (candidate->senderState == recorded) {
            Packet::SenderState *const popped = candidate->popSenderState();
            assert(popped == recorded);
            return;
        }
        Packet::SenderState *state = candidate->senderState;
        for (std::size_t depth = 0;
             state != nullptr && depth < MaxResponseSenderStateDepth;
             ++depth) {
            if (state->predecessor == recorded) {
                state->predecessor = recorded->predecessor;
                return;
            }
            state = state->predecessor;
        }
    };
    detach(packet);
    for (const auto &entry : my_outstanding_pkt_map)
        detach(entry.second.packet);
    for (const auto &address : my_deferred_pkt_map) {
        for (const DeferredPacket &entry : address.second)
            detach(entry.packet);
    }
    const auto detachQueue = [&detach](const auto *queues, unsigned count) {
        for (unsigned queue = 0; queue < count; ++queue) {
            for (const OutstandingPacket &entry : queues[queue])
                detach(entry.packet);
        }
    };
    detachQueue(my_outstanding_indirect_cache_read_pkts, num_cores);
    detachQueue(my_outstanding_indirect_cache_write_pkts, num_cores);
    detachQueue(my_outstanding_stream_cache_read_pkts, num_cores);
    detachQueue(my_outstanding_stream_cache_write_pkts, num_cores);
    detachQueue(my_outstanding_stream_mem_read_pkts, num_cores);
    detachQueue(my_outstanding_stream_mem_write_pkts, num_cores);
    detachQueue(my_outstanding_indirect_mem_read_pkts, num_channels);
    detachQueue(my_outstanding_indirect_mem_write_pkts, num_channels);
    delete recorded;
}

bool
MAA::canCoalesceOutstandingRead(Addr paddr, FuncUnitType func_unit,
                                int maa_id) const
{
    const auto outstanding = my_outstanding_pkt_map.find(paddr);
    if (outstanding == my_outstanding_pkt_map.end() ||
        outstanding->second.virtualRetirement ||
        outstanding->second.logicalResponseManaged ||
        outstanding->second.cmd != MemCmd::ReadReq)
        return false;

    for (size_t i = 0; i < outstanding->second.maaIDs.size(); ++i) {
        if (outstanding->second.maaIDs[i] == maa_id &&
            outstanding->second.funcUnits[i] == func_unit)
            return false;
    }
    const auto deferred = my_deferred_pkt_map.find(paddr);
    return deferred == my_deferred_pkt_map.end() || deferred->second.empty();
}

void
MAA::sendPacket(FuncUnitType funcUnit, int maaID, PacketPtr pkt, Tick tick,
                bool force_cache, bool force_retirement_cache,
                bool bypass_deferred_queue)
{
    Addr paddr = pkt->req->getPaddr();
    const LogicalSenderStateSearch sender_state_search =
        findLogicalSenderStateBounded(pkt->senderState);
    auto *logical_state = sender_state_search.state;
    const SenderStateOwnership sender_state_ownership =
        inspectSenderStateChain(pkt->senderState);
    panic_if(!sender_state_search.complete ||
                 !sender_state_ownership.boundedAcyclic,
             "%s: sender-state stack exceeds bounded ownership proof\n",
             __func__);
    const bool logical_response_managed = logical_state != nullptr;
    const ResponseSenderStateDecision sender_state_decision =
        classifyResponseSenderState(
            {sender_state_ownership.boundedAcyclic, true,
             sender_state_ownership.logicalNodes,
             sender_state_ownership.logicalAtTop, false},
            logical_response_managed);
    panic_if(!sender_state_decision.valid,
             "%s: invalid initial sender-state ownership shape at 0x%lx\n",
             __func__, paddr);
    const LogicalStreamTransactionTag logical_transaction =
        logical_response_managed ? logical_state->tag
                                 : LogicalStreamTransactionTag{};
    auto logical_owner = my_logical_packet_owners.find(pkt);
    if (logical_response_managed) {
        panic_if(pkt->senderState != logical_state ||
                     !senderStateChainExcludes(logical_state->predecessor,
                                               logical_state) ||
                     funcUnit != FuncUnitType::STREAM ||
                     maaID < 0 || static_cast<unsigned>(maaID) >= num_maas ||
                     !logical_transaction.valid() ||
                     logical_transaction.maaID != maaID ||
                     logical_state->lineAddress != paddr,
                 "%s: invalid logical stream packet identity at 0x%lx\n",
                 __func__, paddr);
        panic_if(logical_transaction.action ==
                         LogicalStreamAction::Writeback &&
                     pkt->cmd == MemCmd::WriteReq &&
                     !bypass_deferred_queue,
                 "%s: logical same-line RMW continuation at 0x%lx must "
                 "bypass deferred traffic\n",
                 __func__, paddr);
        const bool fill_read =
            logical_transaction.action == LogicalStreamAction::Fill &&
            pkt->cmd == MemCmd::ReadReq && force_cache &&
            !force_retirement_cache;
        const bool writeback_read =
            logical_transaction.action == LogicalStreamAction::Writeback &&
            pkt->cmd == MemCmd::ReadExReq && force_cache &&
            !force_retirement_cache;
        const bool writeback_continuation =
            logical_transaction.action == LogicalStreamAction::Writeback &&
            pkt->cmd == MemCmd::WriteReq && force_cache &&
            force_retirement_cache && bypass_deferred_queue;
        panic_if(!fill_read && !writeback_read && !writeback_continuation,
                 "%s: logical request at 0x%lx has invalid action/command/"
                 "route/continuation shape\n",
                 __func__, paddr);
        if (logical_owner != my_logical_packet_owners.end()) {
            const LogicalPacketOwner &owner = logical_owner->second;
            panic_if(owner.stream != &streamAccessUnits[maaID] ||
                         owner.maaID != maaID ||
                         owner.transaction != logical_transaction ||
                         owner.address != paddr || owner.command != pkt->cmd ||
                         !senderStateMatchesSnapshot(
                             owner.senderStateOwnership,
                             sender_state_ownership),
                     "%s: logical PacketPtr owner record changed at 0x%lx\n",
                     __func__, paddr);
        }
    } else {
        panic_if(logical_owner != my_logical_packet_owners.end(),
                 "%s: ordinary packet at 0x%lx has logical owner record\n",
                 __func__, paddr);
    }
    panic_if(force_retirement_cache && !force_cache,
             "%s: retirement-cache routing requires force_cache\n", __func__);
    panic_if(pkt->getAddr() != paddr, "%s: paddr 0x%lx and addr 0x%lx do not match for packet %s\n", __func__, paddr, pkt->getAddr(), pkt->print());
    if (logical_response_managed &&
        logical_owner == my_logical_packet_owners.end()) {
        LogicalPacketOwner owner;
        owner.stream = &streamAccessUnits[maaID];
        owner.maaID = maaID;
        owner.transaction = logical_transaction;
        owner.address = paddr;
        owner.command = pkt->cmd;
        owner.senderStateOwnership = sender_state_ownership;
        logical_owner = my_logical_packet_owners.emplace(pkt, owner).first;
    }
    const auto outstanding_it = my_outstanding_pkt_map.find(paddr);
    const auto deferred_it = my_deferred_pkt_map.find(paddr);
    const bool has_deferred_packets =
        deferred_it != my_deferred_pkt_map.end() &&
        !deferred_it->second.empty();
    const bool retirement_owns_address =
        outstanding_it != my_outstanding_pkt_map.end() &&
        outstanding_it->second.virtualRetirement;
    const bool logical_owns_address =
        outstanding_it != my_outstanding_pkt_map.end() &&
        outstanding_it->second.logicalResponseManaged;
    if (!bypass_deferred_queue &&
        (has_deferred_packets || retirement_owns_address ||
         logical_owns_address ||
         (logical_response_managed &&
          outstanding_it != my_outstanding_pkt_map.end()))) {
        DPRINTF(MAAPort,
                "%s: deferring packet %s behind exact-address "
                "retirement serialization at 0x%lx\n",
                __func__, pkt->print(), paddr);
        my_deferred_pkt_map[paddr].push_back(
            {funcUnit, maaID, pkt, tick, force_cache,
             force_retirement_cache, logical_response_managed,
             logical_transaction, sender_state_ownership, paddr, pkt->cmd,
             pkt->getSize()});
        stats.virtual_retirement_native_deferrals += 1;
        if (!retirement_owns_address)
            stats.virtual_retirement_queue_deferrals += 1;
        return;
    }
    LogicalStreamCounterDecision logical_admission{};
    if (logical_response_managed &&
        outstanding_it == my_outstanding_pkt_map.end()) {
        logical_admission = decideLogicalStreamCounterUpdate(
            logicalStreamRequestKind(pkt->cmd),
            LogicalStreamCounterEvent::Enqueued,
            my_num_outstanding_stream_pkts[maaID]);
        if (!logical_admission.valid) {
            const LogicalPacketOwner owner = logical_owner->second;
            owner.stream->rejectLogicalResponse(
                LogicalStreamResponseResult::Invalid);
            if (pkt->cmd == MemCmd::ReadReq || pkt->cmd == MemCmd::ReadExReq)
                owner.stream->abortReadResponse(paddr);
            owner.stream->abortOwnedLogicalResponse(paddr);
            releaseRecordedLogicalSenderState(pkt, owner);
            my_logical_packet_owners.erase(pkt);
            pkt->deleteData();
            delete pkt;
            panic("%s: terminally cleaned saturated logical admission at "
                  "0x%lx\n",
                  __func__, paddr);
        }
    }
    if (outstanding_it != my_outstanding_pkt_map.end()) {
        DPRINTF(MAAPort, "%s: found %s in outstanding packets\n", __func__, pkt->print());
        if (my_outstanding_pkt_map[paddr].cmd == MemCmd::WritebackDirty && pkt->cmd == MemCmd::ReadExReq) {
            DPRINTF(MAAPort, "%s: store to load forwarding for outstanding write packet %s and new read packet %s\n", __func__, my_outstanding_pkt_map[paddr].packet->print(), pkt->print());
            panic_if(my_outstanding_pkt_map[paddr].maaIDs.size() != 1, "%s: multiple units on outstanding write packet %s\n", __func__, my_outstanding_pkt_map[paddr].packet->print());
            panic_if(my_outstanding_pkt_map[paddr].funcUnits[0] != funcUnit || my_outstanding_pkt_map[paddr].maaIDs[0] != maaID, "%s: outstanding write maaID %d, funcUnit %s, packet %s do not match with new read maaID %d, funcUnit %s, packet %s\n", __func__, my_outstanding_pkt_map[paddr].maaIDs[0], func_unit_names[(uint8_t)my_outstanding_pkt_map[paddr].funcUnits[0]], my_outstanding_pkt_map[paddr].packet->print(), maaID, func_unit_names[(uint8_t)funcUnit], pkt->print());
            if (funcUnit == FuncUnitType::INDIRECT) {
                if (my_outstanding_pkt_map[paddr].cached) {
                    indirectAccessUnits[maaID].cacheReadPacketSent(paddr);
                } else {
                    indirectAccessUnits[maaID].memReadPacketSent(paddr);
                }
                panic_if(indirectAccessUnits[maaID].recvData(paddr, my_outstanding_pkt_map[paddr].packet->getPtr<uint8_t>(), my_outstanding_pkt_map[paddr].cached) == false, "%s: received %s but rejected from indirectAccessUnits[%d]\n", __func__, my_outstanding_pkt_map[paddr].packet->print(), maaID);
            } else if (funcUnit == FuncUnitType::STREAM) {
                streamAccessUnits[maaID].readPacketSent(paddr);
                panic_if(streamAccessUnits[maaID].recvData(paddr, my_outstanding_pkt_map[paddr].packet->getPtr<uint8_t>()) == false, "%s: received %s but rejected from streamAccessUnits[%d]\n", __func__, my_outstanding_pkt_map[paddr].packet->print(), maaID);
            } else {
                panic("Invalid func unit type\n");
            }
        } else if (my_outstanding_pkt_map[paddr].cmd == MemCmd::WritebackDirty && pkt->cmd == MemCmd::WritebackDirty) {
            DPRINTF(MAAPort, "%s: store to store replacement for outstanding write packet %s and new write packet %s\n", __func__, my_outstanding_pkt_map[paddr].packet->print(), pkt->print());
            panic_if(my_outstanding_pkt_map[paddr].maaIDs.size() != 1, "%s: multiple units on outstanding write packet %s\n", __func__, my_outstanding_pkt_map[paddr].packet->print());
            panic_if(my_outstanding_pkt_map[paddr].funcUnits[0] != funcUnit || my_outstanding_pkt_map[paddr].maaIDs[0] != maaID, "%s: outstanding write maaID %d, funcUnit %s, packet %s do not match with new write maaID %d, funcUnit %s, packet %s\n", __func__, my_outstanding_pkt_map[paddr].maaIDs[0], func_unit_names[(uint8_t)my_outstanding_pkt_map[paddr].funcUnits[0]], my_outstanding_pkt_map[paddr].packet->print(), maaID, func_unit_names[(uint8_t)funcUnit], pkt->print());
            my_outstanding_pkt_map[paddr].packet->setData(pkt->getPtr<uint8_t>());
            if (funcUnit == FuncUnitType::INDIRECT) {
                if (my_outstanding_pkt_map[paddr].cached) {
                    indirectAccessUnits[maaID].cacheWritePacketSent(paddr);
                } else {
                    indirectAccessUnits[maaID].memWritePacketSent(paddr);
                }
            } else if (funcUnit == FuncUnitType::STREAM) {
                streamAccessUnits[maaID].writePacketSent(paddr);
            } else {
                panic("Invalid func unit type\n");
            }
        } else {
            panic_if(my_outstanding_pkt_map[paddr].cmd != pkt->cmd, "%s Outstanding command %s from packet %s does not match with command %s from packet %s\n", __func__, my_outstanding_pkt_map[paddr].cmd.toString(), my_outstanding_pkt_map[paddr].packet->print(), pkt->cmdString(), pkt->print());
            panic_if(pkt->isWrite(), "%s cannot have duplicated writes %s and %s\n", __func__, my_outstanding_pkt_map[paddr].packet->print(), pkt->print());
            panic_if(pkt->isRead() == false, "%s: packet %s is not read!\n", __func__, pkt->print());
            if (funcUnit == FuncUnitType::STREAM) {
                for (size_t i = 0;
                     i < my_outstanding_pkt_map[paddr].maaIDs.size(); ++i) {
                    if (my_outstanding_pkt_map[paddr].funcUnits[i] !=
                        FuncUnitType::INDIRECT)
                        continue;
                    const int indirect_id =
                        my_outstanding_pkt_map[paddr].maaIDs[i];
                    if (indirectAccessUnits[indirect_id]
                            .hasPendingDirectIndexLine(paddr)) {
                        (*stats.IND_VirtIndexOutstandingMerges[indirect_id])++;
                    }
                }
            }
            for (int i = 0;
                 i < my_outstanding_pkt_map[paddr].maaIDs.size(); i++) {
                panic_if(
                    my_outstanding_pkt_map[paddr].maaIDs[i] == maaID &&
                        my_outstanding_pkt_map[paddr].funcUnits[i] == funcUnit,
                    "%s: maaID %d and funcUnit %s already in the "
                    "outstanding packet %s\n",
                    __func__, maaID, func_unit_names[(uint8_t)funcUnit],
                    pkt->print());
            }
            my_outstanding_pkt_map[paddr].maaIDs.push_back(maaID);
            my_outstanding_pkt_map[paddr].funcUnits.push_back(funcUnit);
            if (my_outstanding_pkt_map[paddr].sent == false) {
                if (my_outstanding_pkt_map[paddr].tick < tick) {
                    my_outstanding_pkt_map[paddr].tick = tick;
                }
                if (funcUnit == FuncUnitType::INDIRECT) {
                    my_num_outstanding_indirect_pkts[maaID]++;
                } else if (funcUnit == FuncUnitType::STREAM) {
                    my_num_outstanding_stream_pkts[maaID]++;
                } else {
                    panic("Invalid func unit type\n");
                }
            } else {
                if (funcUnit == FuncUnitType::INDIRECT) {
                    if (my_outstanding_pkt_map[paddr].cached) {
                        indirectAccessUnits[maaID].cacheReadPacketSent(paddr);
                    } else {
                        indirectAccessUnits[maaID].memReadPacketSent(paddr);
                    }
                } else if (funcUnit == FuncUnitType::STREAM) {
                    streamAccessUnits[maaID].readPacketSent(paddr);
                } else {
                    panic("Invalid func unit type\n");
                }
            }
        }
    } else {
        my_outstanding_pkt_map[paddr] = OutstandingPacket(pkt, paddr, tick, pkt->cmd);
        my_outstanding_pkt_map[paddr].virtualRetirement =
            force_retirement_cache;
        my_outstanding_pkt_map[paddr].logicalResponseManaged =
            logical_response_managed;
        my_outstanding_pkt_map[paddr].logicalTransaction =
            logical_transaction;
        my_outstanding_pkt_map[paddr].senderStateOwnership =
            sender_state_ownership;
        bool hit_cache = true;
        if (force_cache_access == false && force_cache == false) {
            RequestPtr snoop_req = std::make_shared<Request>(pkt->req->getPaddr(), pkt->req->getSize(), pkt->req->getFlags(), pkt->req->requestorId());
            PacketPtr snoop_pkt = new Packet(snoop_req, MemCmd::SnoopReq);
            snoop_pkt->setExpressSnoop();
            snoop_pkt->headerDelay = snoop_pkt->payloadDelay = 0;
            sendSnoopPacketCpu(snoop_pkt);
            hit_cache = snoop_pkt->isBlockCached();
            DPRINTF(MAAPort, "%s: force_cache is false, snoop request for %s determined %s\n", __func__, pkt->print(), hit_cache ? "cached" : "not cached");
            delete snoop_pkt;
        }
        my_outstanding_pkt_map[paddr].maaIDs.push_back(maaID);
        my_outstanding_pkt_map[paddr].funcUnits.push_back(funcUnit);
        int core_id = core_addr(paddr);
        int channel_id = channel_addr(paddr);
        if (funcUnit == FuncUnitType::INDIRECT) {
            my_num_outstanding_indirect_pkts[maaID]++;
            if (hit_cache) {
                my_outstanding_pkt_map[paddr].cached = true;
                if (pkt->isRead()) {
                    my_outstanding_indirect_cache_read_pkts[core_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_indirect_cache_read_pkts[%s\n", __func__, core_id);
                } else if (pkt->isWrite()) {
                    my_outstanding_indirect_cache_write_pkts[core_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_indirect_cache_write_pkts[%s\n", __func__, core_id);
                } else {
                    panic("Invalid packet type\n");
                }
            } else {
                my_outstanding_pkt_map[paddr].cached = false;
                if (pkt->isRead()) {
                    my_outstanding_indirect_mem_read_pkts[channel_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_indirect_mem_read_pkts[%s\n", __func__, channel_id);
                } else if (pkt->isWrite()) {
                    my_outstanding_indirect_mem_write_pkts[channel_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_indirect_mem_write_pkts[%s\n", __func__, channel_id);
                } else {
                    panic("Invalid packet type\n");
                }
            }
        } else if (funcUnit == FuncUnitType::STREAM) {
            if (logical_response_managed) {
                my_num_outstanding_stream_pkts[maaID] =
                    logical_admission.value;
                logical_owner->second.admitted = true;
                logical_owner->second.counterOwned = true;
            } else {
                my_num_outstanding_stream_pkts[maaID]++;
            }
            my_outstanding_pkt_map[paddr].cached = true;
            if (hit_cache) {
                if (pkt->isRead()) {
                    my_outstanding_stream_cache_read_pkts[core_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_stream_cache_read_pkts[%s\n", __func__, core_id);
                } else if (pkt->isWrite()) {
                    my_outstanding_stream_cache_write_pkts[core_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_stream_cache_write_pkts[%s\n", __func__, core_id);
                } else {
                    panic("Invalid packet type\n");
                }
            } else {
                if (pkt->isRead()) {
                    my_outstanding_stream_mem_read_pkts[core_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_stream_mem_read_pkts[%s\n", __func__, core_id);
                } else if (pkt->isWrite()) {
                    my_outstanding_stream_mem_write_pkts[core_id].insert(my_outstanding_pkt_map[paddr]);
                    DPRINTF(MAAPort, "%s: inserting my_outstanding_stream_mem_write_pkts[%s\n", __func__, core_id);
                } else {
                    panic("Invalid packet type\n");
                }
            }
        } else {
            panic("Invalid func unit type\n");
        }
        if (my_outstanding_pkt_map[paddr].cached) {
            scheduleNextSendCache();
        } else {
            scheduleNextSendMem();
        }
    }
}
void MAA::sendNextDeferredPacket(Addr paddr) {
    auto deferred_it = my_deferred_pkt_map.find(paddr);
    if (deferred_it == my_deferred_pkt_map.end() ||
        deferred_it->second.empty()) {
        return;
    }
    const auto occupied = my_outstanding_pkt_map.find(paddr);
    if (occupied != my_outstanding_pkt_map.end()) {
        if (occupied->second.packet != deferred_it->second.front().packet)
            return;
    }

    const DeferredPacket &queued = deferred_it->second.front();
    const bool packet_present = queued.packet != nullptr;
    const bool request_present =
        packet_present && queued.packet->req != nullptr;
    const bool address_matches =
        request_present && queued.paddr == paddr &&
        queued.packet->req->getPaddr() == paddr &&
        queued.packet->getAddr() == paddr;
    const bool command_matches =
        packet_present && queued.packet->cmd == queued.cmd &&
        queued.packet->getSize() == queued.expectedResponseBytes;
    const bool owner_valid =
        (queued.funcUnit == FuncUnitType::STREAM && queued.maaID >= 0 &&
         static_cast<unsigned>(queued.maaID) < num_maas) ||
        (queued.funcUnit == FuncUnitType::INDIRECT && queued.maaID >= 0 &&
         static_cast<unsigned>(queued.maaID) < num_indirect_units_total);
    const bool route_valid =
        (!queued.forceRetirementCache || queued.forceCache) &&
        (!queued.logicalResponseManaged ||
         (queued.funcUnit == FuncUnitType::STREAM && queued.forceCache &&
          ((queued.logicalTransaction.action == LogicalStreamAction::Fill &&
            queued.cmd == MemCmd::ReadReq &&
            !queued.forceRetirementCache) ||
           (queued.logicalTransaction.action ==
                LogicalStreamAction::Writeback &&
            queued.cmd == MemCmd::ReadExReq &&
            !queued.forceRetirementCache) ||
           (queued.logicalTransaction.action ==
                LogicalStreamAction::Writeback &&
            queued.cmd == MemCmd::WriteReq &&
            queued.forceRetirementCache))));
    const auto *logical_state = packet_present
        ? dynamic_cast<LogicalSPDTransactionState *>(
              queued.packet->senderState)
        : nullptr;
    const bool logical_identity_matches = queued.logicalResponseManaged
        ? logical_state != nullptr &&
              logical_state->tag == queued.logicalTransaction &&
              logical_state->lineAddress == paddr
        : logical_state == nullptr && !queued.logicalTransaction.valid();
    const SenderStateOwnership promoted_ownership =
        inspectSenderStateChain(packet_present ? queued.packet->senderState
                                               : nullptr);
    const bool sender_state_matches = packet_present &&
        senderStateMatchesSnapshot(queued.senderStateOwnership,
                                   promoted_ownership);
    const PacketAliasCounts aliases =
        packet_present ? countPacketAliases(queued.packet)
                       : PacketAliasCounts{};
    const bool unique_deferred_owner = aliases.outstanding == 0 &&
        aliases.deferred == 1 && aliases.cacheSend == 0 &&
        aliases.memorySend == 0;
    const auto logical_owner = packet_present
        ? my_logical_packet_owners.find(queued.packet)
        : my_logical_packet_owners.end();
    const LogicalLedgerOwner deferred_ledger =
        queued.logicalResponseManaged && owner_valid
        ? findUniqueLogicalLedgerOwner(queued.logicalTransaction, paddr)
        : LogicalLedgerOwner{};
    const bool logical_admission_capacity =
        !queued.logicalResponseManaged ||
        (owner_valid &&
         deferred_ledger.stream == &streamAccessUnits[queued.maaID] &&
         deferred_ledger.maaID == queued.maaID);
    const bool owner_record_valid = !queued.logicalResponseManaged ||
        (owner_valid && logical_owner != my_logical_packet_owners.end() &&
         logical_owner->second.stream == &streamAccessUnits[queued.maaID] &&
         logical_owner->second.maaID == queued.maaID &&
         logical_owner->second.transaction == queued.logicalTransaction &&
         logical_owner->second.address == paddr &&
         logical_owner->second.command == queued.cmd &&
         !logical_owner->second.admitted &&
         !logical_owner->second.counterOwned &&
         !logical_owner->second.sent);
    const int core = address_matches ? core_addr(paddr) : -1;
    const bool exact_port_valid = !queued.forceCache ||
        (core >= 0 &&
         (queued.forceRetirementCache
              ? static_cast<std::size_t>(core) < retirementSidePorts.size() &&
                    retirementSidePorts[core] != nullptr
              : static_cast<std::size_t>(core) < cacheSidePorts.size() &&
                    cacheSidePorts[core] != nullptr));
    LogicalStreamCounterDecision admission{};
    const bool logical_command_valid = queued.cmd == MemCmd::ReadReq ||
        queued.cmd == MemCmd::ReadExReq || queued.cmd == MemCmd::WriteReq;
    const bool counter_headroom = !queued.logicalResponseManaged ||
        (owner_valid && request_present && logical_command_valid &&
         (admission = decideLogicalStreamCounterUpdate(
              logicalStreamRequestKind(queued.cmd),
              LogicalStreamCounterEvent::Enqueued,
              my_num_outstanding_stream_pkts[queued.maaID])).valid);
    const DeferredPromotionShape promotion_shape = {
        packet_present, request_present, address_matches, command_matches,
        owner_valid && owner_record_valid, route_valid,
        logical_identity_matches, sender_state_matches, exact_port_valid,
        logical_admission_capacity, counter_headroom,
        occupied == my_outstanding_pkt_map.end(), unique_deferred_owner};
    if (!canPromoteDeferredPacket(promotion_shape)) {
        PacketPtr const corrupt = queued.packet;
        const bool cleanup_logical = queued.logicalResponseManaged;
        LogicalPacketOwner owner;
        const bool has_owner = logical_owner != my_logical_packet_owners.end();
        if (has_owner) {
            owner = logical_owner->second;
        } else if (queued.logicalResponseManaged) {
            owner.stream = deferred_ledger.stream;
            owner.maaID = deferred_ledger.maaID;
            owner.transaction = queued.logicalTransaction;
            owner.address = paddr;
            owner.command = queued.cmd;
            owner.senderStateOwnership = queued.senderStateOwnership;
        }
        LogicalStreamCounterDecision aborted_counter{};
        const bool cleanup_command_valid =
            owner.command == MemCmd::ReadReq ||
            owner.command == MemCmd::ReadExReq ||
            owner.command == MemCmd::WriteReq;
        const bool aborted_counter_valid = !owner.counterOwned ||
            (owner.maaID >= 0 &&
             static_cast<unsigned>(owner.maaID) < num_maas &&
             cleanup_command_valid &&
             (aborted_counter = decideLogicalStreamCounterUpdate(
                  logicalStreamRequestKind(owner.command),
                  owner.admitted
                      ? LogicalStreamCounterEvent::ResponseAborted
                      : LogicalStreamCounterEvent::UnsentPacketAborted,
                  my_num_outstanding_stream_pkts[owner.maaID])).valid);
        if (corrupt != nullptr)
            erasePacketAliases(corrupt);
        if (owner.stream != nullptr) {
            owner.stream->rejectLogicalResponse(
                LogicalStreamResponseResult::Invalid);
            if (owner.command == MemCmd::ReadReq ||
                owner.command == MemCmd::ReadExReq) {
                owner.stream->abortReadResponse(owner.address);
            }
            owner.stream->abortOwnedLogicalResponse(owner.address);
        }
        if (aborted_counter_valid && owner.counterOwned) {
            my_num_outstanding_stream_pkts[owner.maaID] =
                aborted_counter.value;
        }
        if (corrupt != nullptr && cleanup_logical) {
            releaseRecordedLogicalSenderState(corrupt, owner);
        }
        if (has_owner)
            my_logical_packet_owners.erase(corrupt);
        if (corrupt != nullptr) {
            corrupt->deleteData();
            delete corrupt;
        }
        panic("%s: terminally cleaned malformed/saturated deferred packet at "
              "0x%lx (pkt=%d req=%d addr=%d cmd=%d owner=%d route=%d "
              "logical=%d sender=%d port=%d counter=%d map=%d alias=%d)\n",
              __func__, paddr, packet_present, request_present,
              address_matches, command_matches, owner_valid &&
              owner_record_valid, route_valid, logical_identity_matches,
              sender_state_matches, exact_port_valid,
              logical_admission_capacity && counter_headroom,
              occupied == my_outstanding_pkt_map.end(),
              unique_deferred_owner);
    }

    DeferredPacket deferred = queued;
    deferred_it->second.pop_front();
    if (deferred_it->second.empty())
        my_deferred_pkt_map.erase(deferred_it);
    DPRINTF(MAAPort, "%s: releasing deferred packet %s at 0x%lx\n",
            __func__, deferred.packet->print(), paddr);
    if (deferred.logicalResponseManaged) {
        OutstandingPacket promoted(deferred.packet, paddr, deferred.tick,
                                   deferred.cmd);
        promoted.cached = true;
        promoted.virtualRetirement = deferred.forceRetirementCache;
        promoted.logicalResponseManaged = true;
        promoted.logicalTransaction = deferred.logicalTransaction;
        promoted.senderStateOwnership = deferred.senderStateOwnership;
        promoted.expectedResponseBytes = deferred.expectedResponseBytes;
        promoted.maaIDs.push_back(deferred.maaID);
        promoted.funcUnits.push_back(FuncUnitType::STREAM);
        my_outstanding_pkt_map.emplace(paddr, promoted);
        my_num_outstanding_stream_pkts[deferred.maaID] = admission.value;
        logical_owner->second.admitted = true;
        logical_owner->second.counterOwned = true;
        if (deferred.packet->isRead()) {
            my_outstanding_stream_cache_read_pkts[core].insert(promoted);
        } else {
            my_outstanding_stream_cache_write_pkts[core].insert(promoted);
        }
        scheduleNextSendCache();
        return;
    }
    sendPacket(deferred.funcUnit, deferred.maaID, deferred.packet,
               deferred.tick, deferred.forceCache,
               deferred.forceRetirementCache, true);
}
bool MAA::scheduleNextSendMem() {
    bool return_val = false;
    Tick tick = 0;
    for (int ch = 0; ch < num_channels; ch++) {
        if (mem_channels_blocked[ch])
            continue;
        if (my_outstanding_indirect_mem_read_pkts[ch].empty() == false) {
            if (return_val == false) {
                tick = my_outstanding_indirect_mem_read_pkts[ch].begin()->tick;
                return_val = true;
            } else {
                tick = std::min(tick, my_outstanding_indirect_mem_read_pkts[ch].begin()->tick);
            }
        }
        if (my_outstanding_indirect_mem_write_pkts[ch].empty() == false) {
            if (return_val == false) {
                tick = my_outstanding_indirect_mem_write_pkts[ch].begin()->tick;
                return_val = true;
            } else {
                tick = std::min(tick, my_outstanding_indirect_mem_write_pkts[ch].begin()->tick);
            }
        }
    }
    if (return_val) {
        Cycles latency = Cycles(0);
        if (tick > curTick()) {
            latency = getTicksToCycles(tick - curTick());
        }
        scheduleSendMemEvent(latency);
    }
    return return_val;
}
bool MAA::allIndirectEmpty() {
    for (int ch = 0; ch < num_channels; ch++) {
        if (my_outstanding_indirect_mem_read_pkts[ch].empty() == false)
            return false;
        if (my_outstanding_indirect_mem_write_pkts[ch].empty() == false)
            return false;
    }
    return true;
}
bool MAA::scheduleNextSendCache() {
    bool return_val = false;
    bool all_indirect_empty = allIndirectEmpty();
    Tick tick = 0;
    for (int core_id = 0; core_id < num_cores; core_id++) {
        if (cache_bus_blocked[core_id])
            continue;
        if (my_outstanding_indirect_cache_read_pkts[core_id].empty() == false) {
            if (return_val == false) {
                tick = my_outstanding_indirect_cache_read_pkts[core_id].begin()->tick;
                return_val = true;
            } else {
                tick = std::min(tick, my_outstanding_indirect_cache_read_pkts[core_id].begin()->tick);
            }
        }
        if (my_outstanding_indirect_cache_write_pkts[core_id].empty() == false) {
            if (return_val == false) {
                tick = my_outstanding_indirect_cache_write_pkts[core_id].begin()->tick;
                return_val = true;
            } else {
                tick = std::min(tick, my_outstanding_indirect_cache_write_pkts[core_id].begin()->tick);
            }
        }
        if (my_outstanding_stream_cache_read_pkts[core_id].empty() == false) {
            if (return_val == false) {
                tick = my_outstanding_stream_cache_read_pkts[core_id].begin()->tick;
                return_val = true;
            } else {
                tick = std::min(tick, my_outstanding_stream_cache_read_pkts[core_id].begin()->tick);
            }
        }
        if (my_outstanding_stream_cache_write_pkts[core_id].empty() == false) {
            if (return_val == false) {
                tick = my_outstanding_stream_cache_write_pkts[core_id].begin()->tick;
                return_val = true;
            } else {
                tick = std::min(tick, my_outstanding_stream_cache_write_pkts[core_id].begin()->tick);
            }
        }
        if (all_indirect_empty) {
            if (my_outstanding_stream_mem_read_pkts[core_id].empty() == false) {
                if (return_val == false) {
                    tick = my_outstanding_stream_mem_read_pkts[core_id].begin()->tick;
                    return_val = true;
                } else {
                    tick = std::min(tick, my_outstanding_stream_mem_read_pkts[core_id].begin()->tick);
                }
            }
            if (my_outstanding_stream_mem_write_pkts[core_id].empty() == false) {
                if (return_val == false) {
                    tick = my_outstanding_stream_mem_write_pkts[core_id].begin()->tick;
                    return_val = true;
                } else {
                    tick = std::min(tick, my_outstanding_stream_mem_write_pkts[core_id].begin()->tick);
                }
            }
        }
    }
    if (return_val) {
        Cycles latency = Cycles(0);
        if (tick > curTick()) {
            latency = getTicksToCycles(tick - curTick());
        }
        scheduleSendCacheEvent(latency);
    }
    return return_val;
}
void MAA::unblockMemChannel(int channel_id) {
    panic_if(mem_channels_blocked[channel_id] == false, "%s: channel %d is not blocked!\n", __func__, channel_id);
    mem_channels_blocked[channel_id] = false;
    scheduleNextSendMem();
}
void MAA::unblockCache(int core_id) {
    panic_if(cache_bus_blocked[core_id] == false, "%s: cache %d is not blocked!\n", __func__, core_id);
    cache_bus_blocked[core_id] = false;
    scheduleNextSendCache();
}
bool MAA::allIndirectPacketsSent(int maaID) {
    return my_num_outstanding_indirect_pkts[maaID] == 0;
}
bool MAA::allStreamPacketsSent(int maaID) {
    return my_num_outstanding_stream_pkts[maaID] == 0;
}
bool MAA::sendOutstandingMemPacket() {
    bool packet_remaining = false;
    bool all_empty = true;
    for (int ch = 0; ch < num_channels; ch++) {
        if (mem_channels_blocked[ch])
            continue;
        // RMW/scatter writebacks are generated by completed reads. Prioritize
        // ready indirect reads so writebacks do not starve the work that keeps
        // the MAA pipeline fed; writes still issue as soon as no ready read is
        // pending on this channel.
        for (auto it = my_outstanding_indirect_mem_read_pkts[ch].begin(); it != my_outstanding_indirect_mem_read_pkts[ch].end();) {
            if (it->tick > curTick()) {
                DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to memory\n", __func__, getTicksToCycles(it->tick - curTick()), it->packet->print());
                packet_remaining = true;
                break;
            }
            DPRINTF(MAAPort, "%s: trying sending %s to memory\n", __func__, it->packet->print());
            if (sendPacketMem(it->packet) == false) {
                DPRINTF(MAAPort, "%s: send failed for channel %d\n", __func__, ch);
                mem_channels_blocked[ch] = true;
                break;
            } else {
                Addr paddr = it->paddr;
                OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                for (int i = 0; i < tmp.maaIDs.size(); i++) {
                    if (tmp.funcUnits[i] == FuncUnitType::INDIRECT) {
                        my_num_outstanding_indirect_pkts[tmp.maaIDs[i]]--;
                        indirectAccessUnits[tmp.maaIDs[i]].memReadPacketSent(it->paddr);
                    } else if (tmp.funcUnits[i] == FuncUnitType::STREAM) {
                        settleAcceptedStreamReadCounter(
                            my_num_outstanding_stream_pkts[tmp.maaIDs[i]],
                            tmp.logicalResponseManaged, tmp.cmd, paddr);
                        streamAccessUnits[tmp.maaIDs[i]].readPacketSent(
                            it->paddr);
                    } else {
                        panic("Invalid func unit type\n");
                    }
                }
                my_outstanding_pkt_map[paddr].sent = true;
                it = my_outstanding_indirect_mem_read_pkts[ch].erase(it);
                stats.port_mem_RD_packets += 1;
            }
        }
        if (mem_channels_blocked[ch])
            continue;
        // Smart writeback queue: reads already drained above, so this loop only
        // runs in slots where no ready read exists -> overlap is preserved. Among
        // the *ready* writebacks (tick <= now), prefer one whose DRAM row is still
        // open in its bank (the last write to that bank touched the same row) so
        // it issues as a row-buffer hit. Fall back to the oldest ready write when
        // no open-row match exists. This row-groups the write stream without
        // serializing it behind the reads.
        std::multiset<OutstandingPacket, CompareByTick> &wq = my_outstanding_indirect_mem_write_pkts[ch];
        while (wq.empty() == false) {
            auto oldest = wq.begin();
            if (oldest->tick > curTick()) {
                DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to memory\n", __func__, getTicksToCycles(oldest->tick - curTick()), oldest->packet->print());
                packet_remaining = true;
                break;
            }
            // Scan the ready prefix (ordered by tick) for an open-row hit.
            auto chosen = oldest;
            bool chosen_hit = false;
            for (auto it = wq.begin(); it != wq.end() && it->tick <= curTick(); ++it) {
                uint64_t bank_key;
                Addr row;
                writeRowKey(it->paddr, bank_key, row);
                auto lr = my_writeback_last_row[ch].find(bank_key);
                if (lr != my_writeback_last_row[ch].end() && lr->second == row) {
                    chosen = it;
                    chosen_hit = true;
                    break;
                }
            }
            DPRINTF(MAAPort, "%s: trying sending %s to memory (row-%s)\n", __func__, chosen->packet->print(), chosen_hit ? "hit" : "miss");
            if (sendPacketMem(chosen->packet) == false) {
                DPRINTF(MAAPort, "%s: send failed for channel %d\n", __func__, ch);
                mem_channels_blocked[ch] = true;
                break;
            } else {
                Addr paddr = chosen->paddr;
                panic_if(chosen->packet->needsResponse(), "%s write packet %s needs response!\n", __func__, chosen->packet->print());
                OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                my_outstanding_pkt_map.erase(paddr);
                panic_if(tmp.maaIDs.size() != 1, "%s multiple write packes coalesced into one!\n", __func__);
                panic_if(tmp.funcUnits[0] != FuncUnitType::INDIRECT, "%s: func unit type %d does not match with %d\n", __func__, func_unit_names[(uint8_t)tmp.funcUnits[0]], func_unit_names[(uint8_t)FuncUnitType::INDIRECT]);
                my_num_outstanding_indirect_pkts[tmp.maaIDs[0]]--;
                sendNextDeferredPacket(paddr);
                indirectAccessUnits[tmp.maaIDs[0]].memWritePacketSent(paddr);
                uint64_t bank_key;
                Addr row;
                writeRowKey(paddr, bank_key, row);
                my_writeback_last_row[ch][bank_key] = row;
                if (chosen_hit)
                    stats.port_mem_WR_rowhit += 1;
                wq.erase(chosen);
                stats.port_mem_WR_packets += 1;
            }
        }
        if (my_outstanding_indirect_mem_read_pkts[ch].empty() == false || my_outstanding_indirect_mem_write_pkts[ch].empty() == false) {
            all_empty = false;
        }
    }
    if (packet_remaining) {
        scheduleNextSendMem();
    }
    if (all_empty) {
        scheduleNextSendCache();
    }
    return true;
}
bool MAA::sendOutstandingCachePacket() {
    bool packet_remaining = false;
    bool all_indirect_empty = allIndirectEmpty();
    for (int core = 0; core < num_cores; core++) {
        if (cache_bus_blocked[core])
            continue;
        // Same policy as memory-side: keep indirect reads ahead of writebacks
        // so write responses do not block the reads that create future work.
        for (auto it = my_outstanding_indirect_cache_read_pkts[core].begin(); it != my_outstanding_indirect_cache_read_pkts[core].end();) {
            if (it->tick > curTick()) {
                DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to cache\n", __func__, getTicksToCycles(it->tick - curTick()), it->packet->print());
                packet_remaining = true;
                break;
            }
            DPRINTF(MAAPort, "%s: trying sending %s to cache\n", __func__, it->packet->print());
            CacheSidePort *sending_port = nullptr;
            if (!sendPacketCache(it->packet, &sending_port)) {
                DPRINTF(MAAPort, "%s: send failed for bus %d\n", __func__,
                        core);
                cache_bus_blocked[core] = true;
                break;
            } else {
                Addr paddr = it->paddr;
                OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                for (int i = 0; i < tmp.maaIDs.size(); i++) {
                    if (tmp.funcUnits[i] == FuncUnitType::INDIRECT) {
                        my_num_outstanding_indirect_pkts[tmp.maaIDs[i]]--;
                        indirectAccessUnits[tmp.maaIDs[i]].cacheReadPacketSent(
                            it->paddr);
                    } else if (tmp.funcUnits[i] == FuncUnitType::STREAM) {
                        settleAcceptedStreamReadCounter(
                            my_num_outstanding_stream_pkts[tmp.maaIDs[i]],
                            tmp.logicalResponseManaged, tmp.cmd, paddr);
                        streamAccessUnits[tmp.maaIDs[i]].readPacketSent(
                            it->paddr);
                    } else {
                        panic("Invalid func unit type\n");
                    }
                }
                my_outstanding_pkt_map[paddr].responseCreditPort =
                    sending_port;
                my_outstanding_pkt_map[paddr].sent = true;
                it = my_outstanding_indirect_cache_read_pkts[core].erase(it);
                stats.port_cache_RD_packets += 1;
            }
        }
        if (cache_bus_blocked[core])
            continue;
        for (auto it = my_outstanding_indirect_cache_write_pkts[core].begin(); it != my_outstanding_indirect_cache_write_pkts[core].end();) {
            if (it->tick > curTick()) {
                DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to cache\n", __func__, getTicksToCycles(it->tick - curTick()), it->packet->print());
                packet_remaining = true;
                break;
            }
            DPRINTF(MAAPort, "%s: trying sending %s to cache\n", __func__, it->packet->print());
            const bool needs_response = it->packet->needsResponse();
            CacheSidePort *sending_port = nullptr;
            const bool sent = it->virtualRetirement
                ? sendPacketRetirementCache(it->packet, &sending_port)
                : sendPacketCache(it->packet, &sending_port);
            if (!sent) {
                DPRINTF(MAAPort, "%s: send failed for bus %d\n", __func__,
                        core);
                cache_bus_blocked[core] = true;
                break;
            } else {
                Addr paddr = it->paddr;
                OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                panic_if(tmp.maaIDs.size() != 1, "%s multiple write packes coalesced into one!\n", __func__);
                panic_if(tmp.funcUnits[0] != FuncUnitType::INDIRECT, "%s: func unit type %d does not match with %d\n", __func__, func_unit_names[(uint8_t)tmp.funcUnits[0]], func_unit_names[(uint8_t)FuncUnitType::INDIRECT]);
                if (needs_response) {
                    my_outstanding_pkt_map[paddr].responseCreditPort =
                        sending_port;
                    my_outstanding_pkt_map[paddr].sent = true;
                    auto owner = my_logical_packet_owners.find(tmp.packet);
                    panic_if(owner == my_logical_packet_owners.end(),
                             "%s: logical write at 0x%lx lost PacketPtr "
                             "owner record\n",
                             __func__, paddr);
                    owner->second.sent = true;
                    owner->second.responseCreditPort = sending_port;
                } else {
                    my_outstanding_pkt_map.erase(paddr);
                    my_num_outstanding_indirect_pkts[tmp.maaIDs[0]]--;
                    sendNextDeferredPacket(paddr);
                    indirectAccessUnits[tmp.maaIDs[0]].cacheWritePacketSent(it->paddr);
                }
                it = my_outstanding_indirect_cache_write_pkts[core].erase(it);
                stats.port_cache_WR_packets += 1;
            }
        }
        if (cache_bus_blocked[core])
            continue;
        for (auto it = my_outstanding_stream_cache_write_pkts[core].begin(); it != my_outstanding_stream_cache_write_pkts[core].end();) {
            if (it->tick > curTick()) {
                DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to cache\n", __func__, getTicksToCycles(it->tick - curTick()), it->packet->print());
                packet_remaining = true;
                break;
            }
            DPRINTF(MAAPort, "%s: trying sending %s to cache\n", __func__, it->packet->print());
            const bool needs_response = it->packet->needsResponse();
            CacheSidePort *sending_port = nullptr;
            const bool sent = it->virtualRetirement
                ? sendPacketRetirementCache(it->packet, &sending_port)
                : sendPacketCache(it->packet, &sending_port);
            if (!sent) {
                DPRINTF(MAAPort, "%s: send failed for bus %d\n", __func__,
                        core);
                cache_bus_blocked[core] = true;
                break;
            } else {
                Addr paddr = it->paddr;
                OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                panic_if(tmp.maaIDs.size() != 1,
                         "%s multiple write packes coalesced into one!\n",
                         __func__);
                panic_if(tmp.funcUnits[0] != FuncUnitType::STREAM,
                         "%s: func unit type %d does not match with %d\n",
                         __func__,
                         func_unit_names[(uint8_t)tmp.funcUnits[0]],
                         func_unit_names[(uint8_t)FuncUnitType::STREAM]);
                if (tmp.logicalResponseManaged) {
                    panic_if(!needs_response ||
                                 tmp.logicalTransaction.action !=
                                     LogicalStreamAction::Writeback ||
                                 tmp.cmd != MemCmd::WriteReq ||
                                 !tmp.virtualRetirement,
                             "%s: logical stream write at 0x%lx lost its "
                             "response-bearing retirement identity\n",
                             __func__, paddr);
                    applyLogicalStreamCounterEvent(
                        my_num_outstanding_stream_pkts[tmp.maaIDs[0]],
                        tmp.cmd, LogicalStreamCounterEvent::SendAccepted,
                        paddr);
                    my_outstanding_pkt_map[paddr].responseCreditPort =
                        sending_port;
                    my_outstanding_pkt_map[paddr].sent = true;
                } else {
                    panic_if(needs_response,
                             "%s write packet %s needs response!\n",
                             __func__, it->packet->print());
                    my_outstanding_pkt_map.erase(paddr);
                    my_num_outstanding_stream_pkts[tmp.maaIDs[0]]--;
                    sendNextDeferredPacket(paddr);
                    streamAccessUnits[tmp.maaIDs[0]].writePacketSent(
                        it->paddr);
                }
                it = my_outstanding_stream_cache_write_pkts[core].erase(it);
                stats.port_cache_WR_packets += 1;
            }
        }
        if (cache_bus_blocked[core])
            continue;
        for (auto it = my_outstanding_stream_cache_read_pkts[core].begin(); it != my_outstanding_stream_cache_read_pkts[core].end();) {
            if (it->tick > curTick()) {
                DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to cache\n", __func__, getTicksToCycles(it->tick - curTick()), it->packet->print());
                packet_remaining = true;
                break;
            }
            DPRINTF(MAAPort, "%s: trying sending %s to cache\n", __func__, it->packet->print());
            CacheSidePort *sending_port = nullptr;
            if (!sendPacketCache(it->packet, &sending_port)) {
                DPRINTF(MAAPort, "%s: send failed for bus %d\n", __func__,
                        core);
                cache_bus_blocked[core] = true;
                break;
            } else {
                Addr paddr = it->paddr;
                OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                for (int i = 0; i < tmp.maaIDs.size(); i++) {
                    if (tmp.funcUnits[i] == FuncUnitType::INDIRECT) {
                        my_num_outstanding_indirect_pkts[tmp.maaIDs[i]]--;
                        indirectAccessUnits[tmp.maaIDs[i]].cacheReadPacketSent(it->paddr);
                    } else if (tmp.funcUnits[i] == FuncUnitType::STREAM) {
                        settleAcceptedStreamReadCounter(
                            my_num_outstanding_stream_pkts[tmp.maaIDs[i]],
                            tmp.logicalResponseManaged, tmp.cmd, paddr);
                        streamAccessUnits[tmp.maaIDs[i]].readPacketSent(
                            it->paddr);
                        if (tmp.logicalResponseManaged) {
                            auto owner = my_logical_packet_owners.find(
                                tmp.packet);
                            panic_if(owner == my_logical_packet_owners.end(),
                                     "%s: logical read at 0x%lx lost "
                                     "PacketPtr owner record\n",
                                     __func__, paddr);
                            owner->second.sent = true;
                            owner->second.counterOwned = false;
                            owner->second.responseCreditPort = sending_port;
                        }
                    } else {
                        panic("Invalid func unit type\n");
                    }
                }
                my_outstanding_pkt_map[paddr].responseCreditPort =
                    sending_port;
                my_outstanding_pkt_map[paddr].sent = true;
                it = my_outstanding_stream_cache_read_pkts[core].erase(it);
                stats.port_cache_RD_packets += 1;
            }
        }
        if (all_indirect_empty) {
            if (cache_bus_blocked[core])
                continue;
            for (auto it = my_outstanding_stream_mem_write_pkts[core].begin(); it != my_outstanding_stream_mem_write_pkts[core].end();) {
                if (it->tick > curTick()) {
                    DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to cache\n", __func__, getTicksToCycles(it->tick - curTick()), it->packet->print());
                    packet_remaining = true;
                    break;
                }
                DPRINTF(MAAPort, "%s: trying sending %s to cache\n", __func__, it->packet->print());
                if (sendPacketCache(it->packet) == false) {
                    DPRINTF(MAAPort, "%s: send failed for bus %d\n", __func__,
                            core);
                    cache_bus_blocked[core] = true;
                    break;
                } else {
                    Addr paddr = it->paddr;
                    panic_if(it->packet->needsResponse(), "%s write packet %s needs response!\n", __func__, it->packet->print());
                    OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                    my_outstanding_pkt_map.erase(paddr);
                    panic_if(tmp.maaIDs.size() != 1, "%s multiple write packes coalesced into one!\n", __func__);
                    panic_if(tmp.funcUnits[0] != FuncUnitType::STREAM, "%s: func unit type %d does not match with %d\n", __func__, func_unit_names[(uint8_t)tmp.funcUnits[0]], func_unit_names[(uint8_t)FuncUnitType::STREAM]);
                    my_num_outstanding_stream_pkts[tmp.maaIDs[0]]--;
                    sendNextDeferredPacket(paddr);
                    streamAccessUnits[tmp.maaIDs[0]].writePacketSent(it->paddr);
                    it = my_outstanding_stream_mem_write_pkts[core].erase(it);
                    stats.port_cache_WR_packets += 1;
                }
            }
            if (cache_bus_blocked[core])
                continue;
            for (auto it = my_outstanding_stream_mem_read_pkts[core].begin(); it != my_outstanding_stream_mem_read_pkts[core].end();) {
                if (it->tick > curTick()) {
                    DPRINTF(MAAPort, "%s: waiting for %d cycles to send %s to cache\n", __func__, getTicksToCycles(it->tick - curTick()), it->packet->print());
                    packet_remaining = true;
                    break;
                }
                DPRINTF(MAAPort, "%s: trying sending %s to cache\n", __func__, it->packet->print());
                CacheSidePort *sending_port = nullptr;
                if (!sendPacketCache(it->packet, &sending_port)) {
                    DPRINTF(MAAPort, "%s: send failed for bus %d\n", __func__,
                            core);
                    cache_bus_blocked[core] = true;
                    break;
                } else {
                    Addr paddr = it->paddr;
                    OutstandingPacket tmp = my_outstanding_pkt_map[paddr];
                    for (int i = 0; i < tmp.maaIDs.size(); i++) {
                        if (tmp.funcUnits[i] == FuncUnitType::INDIRECT) {
                            my_num_outstanding_indirect_pkts[tmp.maaIDs[i]]--;
                            indirectAccessUnits[tmp.maaIDs[i]]
                                .cacheReadPacketSent(it->paddr);
                        } else if (tmp.funcUnits[i] == FuncUnitType::STREAM) {
                            settleAcceptedStreamReadCounter(
                                my_num_outstanding_stream_pkts[tmp.maaIDs[i]],
                                tmp.logicalResponseManaged, tmp.cmd, paddr);
                            streamAccessUnits[tmp.maaIDs[i]].readPacketSent(
                                it->paddr);
                            if (tmp.logicalResponseManaged) {
                                auto owner = my_logical_packet_owners.find(
                                    tmp.packet);
                                panic_if(
                                    owner == my_logical_packet_owners.end(),
                                    "%s: logical read at 0x%lx lost "
                                    "PacketPtr owner record\n",
                                    __func__, paddr);
                                owner->second.sent = true;
                                owner->second.counterOwned = false;
                                owner->second.responseCreditPort =
                                    sending_port;
                            }
                        } else {
                            panic("Invalid func unit type\n");
                        }
                    }
                    my_outstanding_pkt_map[paddr].responseCreditPort =
                        sending_port;
                    my_outstanding_pkt_map[paddr].sent = true;
                    it = my_outstanding_stream_mem_read_pkts[core].erase(it);
                    stats.port_cache_RD_packets += 1;
                }
            }
        }
    }

    if (packet_remaining) {
        scheduleNextSendCache();
    }
    return true;
}
TimingResponseDisposition
MAA::recvTimingResp(PacketPtr pkt, CacheSidePort *responsePort)
{
    DPRINTF(MAAPort, "%s: received %s, cmd: %s, size: %d\n", __func__,
            pkt->print(), pkt->cmdString(), pkt->getSize());

    const Addr response_paddr = pkt->req->getPaddr();
    const Addr response_address = pkt->getAddr();
    LogicalStreamResponseKind response_kind =
        LogicalStreamResponseKind::Read;
    const bool response_command_valid =
        decodeLogicalStreamResponseKind(pkt->cmd, response_kind);
    const SenderStateOwnership received_ownership =
        inspectSenderStateChain(pkt->senderState);
    const LogicalSenderStateSearch sender_state_search =
        findLogicalSenderStateBounded(pkt->senderState);
    auto *logical_state = sender_state_search.state;
    const LogicalStreamTransactionTag received_tag =
        logical_state != nullptr ? logical_state->tag
                                 : LogicalStreamTransactionTag{};
    const Addr sender_line_address =
        logical_state != nullptr ? logical_state->lineAddress : 0;

    const auto exact = std::find_if(
        my_outstanding_pkt_map.begin(), my_outstanding_pkt_map.end(),
        [pkt](const auto &entry) { return entry.second.packet == pkt; });
    const auto address_outstanding =
        my_outstanding_pkt_map.find(response_paddr);
    const auto authoritative_owner = my_logical_packet_owners.find(pkt);
    const PacketAliasCounts aliases_before = countPacketAliases(pkt);
    const bool sender_state_aliased = senderStateAliasedByOtherPacket(
        pkt, received_ownership);
    const bool logical_state_release_safe =
        received_ownership.boundedAcyclic &&
        received_ownership.logicalNodes == 1 &&
        received_ownership.logicalAtTop && !sender_state_aliased &&
        logical_state != nullptr && pkt->senderState == logical_state;
    auto releaseLogicalState = [&](bool authorized) {
        panic_if(!authorized || logical_state == nullptr,
                 "%s: attempted unsafe logical sender-state release\n",
                 __func__);
        panic_if(pkt->senderState != logical_state,
                 "%s: logical sender state is not response-stack top\n",
                 __func__);
        auto *popped = pkt->popSenderState();
        panic_if(popped != logical_state,
                 "%s: logical sender state changed during response\n",
                 __func__);
        delete popped;
        logical_state = nullptr;
    };

    /*
     * No packet other than the exact map-owned PacketPtr can settle the
     * active address.  A separately owned tagged callback is consumed after
     * releasing its own state; an untagged or aliased extra is fatal.  Every
     * MAA production alias is detached before the wrapper destroys a fatal
     * packet.  Deferred/send queues are real owners, not test registries.
    */
    if (exact == my_outstanding_pkt_map.end()) {
        if (authoritative_owner != my_logical_packet_owners.end()) {
            const LogicalPacketOwner owner = authoritative_owner->second;
            LogicalStreamCounterDecision counter{};
            const bool counter_valid = !owner.counterOwned ||
                ((counter = decideLogicalStreamCounterUpdate(
                      logicalStreamRequestKind(owner.command),
                      owner.admitted
                          ? LogicalStreamCounterEvent::ResponseAborted
                          : LogicalStreamCounterEvent::UnsentPacketAborted,
                      my_num_outstanding_stream_pkts[owner.maaID])).valid);
            const TimingResponseCreditOwner credit_owner =
                classifyTimingResponseCreditOwner(
                    owner.sent, owner.responseCreditPort != nullptr,
                    responsePort == owner.responseCreditPort);
            const bool credit_valid = owner.responseCreditPort == nullptr ||
                owner.responseCreditPort->canSettleOwnedResponseCredit();
            if (owner.stream != nullptr) {
                owner.stream->rejectLogicalResponse(
                    LogicalStreamResponseResult::Invalid);
                if (owner.command == MemCmd::ReadReq ||
                    owner.command == MemCmd::ReadExReq) {
                    owner.stream->abortReadResponse(owner.address);
                }
                owner.stream->abortOwnedLogicalResponse(owner.address);
            }
            erasePacketAliases(pkt);
            if (counter_valid && owner.counterOwned) {
                my_num_outstanding_stream_pkts[owner.maaID] = counter.value;
            }
            releaseRecordedLogicalSenderState(pkt, owner);
            my_logical_packet_owners.erase(pkt);
            if (credit_valid &&
                credit_owner ==
                    TimingResponseCreditOwner::ExpectedCachePort) {
                owner.responseCreditPort->settleOwnedResponseCredit();
            }
            return credit_owner == TimingResponseCreditOwner::CallbackPort
                ? TimingResponseDisposition::FatalOwnedCorruption
                : TimingResponseDisposition::FatalOwnedNoPortCredit;
        }
        const LogicalPacketProvenance provenance =
            findLogicalPacketProvenance(pkt);
        const LogicalLedgerOwner orphan_owner =
            provenance.ambiguous
            ? LogicalLedgerOwner{}
            : findUniqueLogicalLedgerOwner(provenance.transaction,
                                           provenance.address);
        const OrphanedLogicalPacketDecision orphan_decision =
            classifyOrphanedLogicalPacket(
                {provenance.deferredAliases, provenance.sendAliases,
                 orphan_owner.stream != nullptr && !provenance.ambiguous});
        if (orphan_decision.abortLedger) {
            LogicalStreamResponseKind request_kind =
                LogicalStreamResponseKind::Read;
            const bool request_kind_valid =
                provenance.command == MemCmd::ReadReq ||
                provenance.command == MemCmd::ReadExReq ||
                provenance.command == MemCmd::WriteReq;
            if (provenance.command == MemCmd::ReadExReq)
                request_kind = LogicalStreamResponseKind::ReadEx;
            else if (provenance.command == MemCmd::WriteReq)
                request_kind = LogicalStreamResponseKind::Write;
            LogicalStreamCounterDecision counter{};
            const bool counter_valid = !orphan_decision.settleCounter ||
                (request_kind_valid &&
                 (counter = decideLogicalStreamCounterUpdate(
                      request_kind,
                      LogicalStreamCounterEvent::UnsentPacketAborted,
                      my_num_outstanding_stream_pkts[
                          orphan_owner.maaID])).valid);
            orphan_owner.stream->rejectLogicalResponse(
                LogicalStreamResponseResult::Invalid);
            if (provenance.command == MemCmd::ReadReq ||
                provenance.command == MemCmd::ReadExReq) {
                orphan_owner.stream->abortReadResponse(provenance.address);
            }
            orphan_owner.stream->abortOwnedLogicalResponse(
                provenance.address);
            if (orphan_decision.settleCounter && counter_valid) {
                my_num_outstanding_stream_pkts[orphan_owner.maaID] =
                    counter.value;
            }
            erasePacketAliases(pkt);
            if (logical_state_release_safe)
                releaseLogicalState(true);
            DPRINTF(MAAPort,
                    "%s: orphaned logical packet at 0x%lx settled from "
                    "deferred/send provenance before fatal destruction\n",
                    __func__, provenance.address);
            return TimingResponseDisposition::FatalUnownedExtra;
        }

        StreamAccessUnit *received_stream = nullptr;
        LogicalStreamResponseResult ledger_result =
            response_command_valid ? LogicalStreamResponseResult::Stale
                                   : LogicalStreamResponseResult::Invalid;
        if (logical_state != nullptr) {
            const LogicalLedgerOwner received_owner =
                findUniqueLogicalLedgerOwner(received_tag,
                                             sender_line_address);
            received_stream = received_owner.stream;
            if (response_command_valid) {
                if (received_stream != nullptr) {
                    ledger_result = received_stream->validateLogicalResponse(
                        received_tag, sender_line_address, response_kind);
                }
            }
        }

        bool address_is_logical = false;
        LogicalStreamTransactionTag expected_tag{};
        Addr outstanding_address = response_paddr;
        LogicalStreamResponseKind expected_kind = response_kind;
        if (address_outstanding != my_outstanding_pkt_map.end()) {
            const OutstandingPacket &address_owner =
                address_outstanding->second;
            outstanding_address = address_outstanding->first;
            address_is_logical = address_owner.logicalResponseManaged;
            expected_tag = address_owner.logicalTransaction;
            if (address_owner.cmd == MemCmd::ReadReq) {
                expected_kind = LogicalStreamResponseKind::Read;
            } else if (address_owner.cmd == MemCmd::ReadExReq) {
                expected_kind = LogicalStreamResponseKind::ReadEx;
            } else if (address_owner.cmd == MemCmd::WriteReq) {
                expected_kind = LogicalStreamResponseKind::Write;
            } else {
                ledger_result = LogicalStreamResponseResult::Invalid;
            }
        }
        const LogicalStreamResponseRoute route = {
            address_outstanding != my_outstanding_pkt_map.end(),
            address_is_logical,
            logical_state != nullptr,
            expected_tag,
            received_tag,
            outstanding_address,
            sender_line_address,
            response_address,
            expected_kind,
            response_kind,
            ledger_result};
        const ResponsePacketAliasDecision extra_alias_decision =
            classifyResponsePacketAliases(
                {aliases_before.outstanding, aliases_before.deferred,
                 aliases_before.cacheSend, aliases_before.memorySend},
                false, false);
        const bool locally_unaliased = extra_alias_decision.droppableExtra;
        const LogicalStreamResponseDispositionDecision decision =
            classifyLogicalStreamResponseDisposition(
                route, false,
                logical_state_release_safe && locally_unaliased);
        if (received_stream != nullptr &&
            decision.result != LogicalStreamResponseResult::Accepted &&
            decision.result != LogicalStreamResponseResult::Completed) {
            received_stream->rejectLogicalResponse(decision.result);
        }
        if (extra_alias_decision.detachBeforeDestruction)
            erasePacketAliases(pkt);
        if (decision.popSenderState ||
            (extra_alias_decision.detachBeforeDestruction &&
             logical_state_release_safe)) {
            releaseLogicalState(true);
        }
        DPRINTF(MAAPort,
                "%s: non-owning response at 0x%lx classified %d with "
                "disposition %d; active map entry unchanged\n",
                __func__, response_paddr,
                static_cast<int>(decision.result),
                static_cast<int>(decision.disposition));
        return decision.disposition;
    }

    const Addr owned_address = exact->first;
    const OutstandingPacket tmp = exact->second;
    const bool has_authoritative_owner =
        authoritative_owner != my_logical_packet_owners.end();
    const LogicalPacketOwner recorded_owner = has_authoritative_owner
        ? authoritative_owner->second : LogicalPacketOwner{};
    CacheSidePort *const expected_response_port = has_authoritative_owner
        ? recorded_owner.responseCreditPort : tmp.responseCreditPort;
    const bool request_sent = has_authoritative_owner
        ? recorded_owner.sent : tmp.sent;
    const bool response_port_matches =
        request_sent && (expected_response_port != nullptr
                         ? responsePort == expected_response_port
                         : responsePort == nullptr);
    const ResponsePacketAliasDecision alias_decision =
        classifyResponsePacketAliases(
            {aliases_before.outstanding, aliases_before.deferred,
             aliases_before.cacheSend, aliases_before.memorySend},
            true, tmp.sent);
    const bool exact_sender_snapshot = senderStateMatchesSnapshot(
        tmp.senderStateOwnership, received_ownership);
    const ResponseSenderStateDecision sender_state_decision =
        classifyResponseSenderState(
            {received_ownership.boundedAcyclic, exact_sender_snapshot,
             received_ownership.logicalNodes,
             received_ownership.logicalAtTop, sender_state_aliased},
            tmp.logicalResponseManaged);
    const auto fatalOwnedDisposition = [&]() {
        const TimingResponseCreditOwner credit_owner =
            classifyTimingResponseCreditOwner(
                request_sent, expected_response_port != nullptr,
                responsePort == expected_response_port);
        if (credit_owner ==
                TimingResponseCreditOwner::ExpectedCachePort &&
            expected_response_port->canSettleOwnedResponseCredit()) {
            expected_response_port->settleOwnedResponseCredit();
        }
        return credit_owner == TimingResponseCreditOwner::CallbackPort
            ? TimingResponseDisposition::FatalOwnedCorruption
            : TimingResponseDisposition::FatalOwnedNoPortCredit;
    };
    if (has_authoritative_owner || tmp.logicalResponseManaged) {
        const LogicalLedgerOwner ledger_owner = has_authoritative_owner
            ? LogicalLedgerOwner{recorded_owner.stream,
                                 recorded_owner.maaID}
            : findUniqueLogicalLedgerOwner(tmp.logicalTransaction,
                                           owned_address);
        const bool owner_metadata_valid =
            tmp.maaIDs.size() == 1 && tmp.funcUnits.size() == 1 &&
            tmp.funcUnits[0] == FuncUnitType::STREAM &&
            tmp.maaIDs[0] >= 0 &&
            static_cast<unsigned>(tmp.maaIDs[0]) < num_maas &&
            tmp.maaIDs[0] == ledger_owner.maaID;
        const bool owner_valid =
            owner_metadata_valid && ledger_owner.stream != nullptr &&
            (!has_authoritative_owner ||
             (recorded_owner.address == owned_address &&
              recorded_owner.transaction == tmp.logicalTransaction &&
              recorded_owner.command == tmp.cmd &&
              recorded_owner.stream == ledger_owner.stream &&
              recorded_owner.maaID == ledger_owner.maaID &&
              recorded_owner.admitted));
        StreamAccessUnit *const stream = ledger_owner.stream;
        LogicalStreamResponseKind expected_kind =
            LogicalStreamResponseKind::Read;
        const bool request_command_valid =
            tmp.cmd == MemCmd::ReadReq || tmp.cmd == MemCmd::ReadExReq ||
            tmp.cmd == MemCmd::WriteReq;
        if (tmp.cmd == MemCmd::ReadExReq)
            expected_kind = LogicalStreamResponseKind::ReadEx;
        else if (tmp.cmd == MemCmd::WriteReq)
            expected_kind = LogicalStreamResponseKind::Write;

        LogicalStreamCounterDecision response_counter{};
        bool response_counter_valid = false;
        if (stream != nullptr && request_command_valid) {
            response_counter = decideLogicalStreamCounterUpdate(
                logicalStreamRequestKind(tmp.cmd),
                LogicalStreamCounterEvent::ResponseAccepted,
                my_num_outstanding_stream_pkts[ledger_owner.maaID]);
            response_counter_valid = response_counter.valid;
        }
        LogicalStreamCounterDecision continuation_counter{};
        const bool continuation_counter_valid =
            tmp.cmd != MemCmd::ReadExReq ||
            (stream != nullptr &&
             (continuation_counter = decideLogicalStreamCounterUpdate(
                  LogicalStreamResponseKind::Write,
                  LogicalStreamCounterEvent::Enqueued,
                  my_num_outstanding_stream_pkts[ledger_owner.maaID])).valid);
        const bool response_credit_valid = expected_response_port == nullptr ||
            expected_response_port->canSettleOwnedResponseCredit();
        const bool unit_owner_valid = stream != nullptr &&
            (tmp.cmd == MemCmd::WriteReq ||
             stream->hasReadResponseOwner(owned_address));

        const bool response_size_valid =
            tmp.expectedResponseBytes == ExpectedLogicalStreamResponseBytes &&
            hasExpectedLogicalStreamResponseSize(
                pkt->getSize(), tmp.expectedResponseBytes);
        const bool structure_valid =
            owner_valid && request_command_valid && response_command_valid &&
            response_size_valid && tmp.sent && response_port_matches &&
            tmp.paddr == owned_address && response_paddr == owned_address &&
            response_address == owned_address &&
            alias_decision.acceptedExactOwner && sender_state_decision.valid &&
            response_counter_valid && continuation_counter_valid &&
            unit_owner_valid &&
            response_credit_valid;
        LogicalStreamResponseResult ledger_result =
            logical_state == nullptr ? LogicalStreamResponseResult::Stale
                                     : LogicalStreamResponseResult::Invalid;
        if (structure_valid) {
            ledger_result = stream->validateLogicalResponse(
                received_tag, sender_line_address, response_kind);
        }
        const LogicalStreamResponseRoute route = {
            true,
            true,
            logical_state != nullptr,
            tmp.logicalTransaction,
            received_tag,
            owned_address,
            sender_line_address,
            response_address,
            expected_kind,
            response_kind,
            ledger_result};
        LogicalStreamResponseDispositionDecision decision =
            classifyLogicalStreamResponseDisposition(
                route, true, sender_state_decision.valid);
        if (!structure_valid && decision.accepts()) {
            decision = {LogicalStreamResponseResult::Invalid,
                        TimingResponseDisposition::FatalOwnedCorruption, true,
                        sender_state_decision.releaseLogicalTop,
                        true};
        }
        const ResponseMutationPreflight response_preflight = {
            owner_valid && unit_owner_valid,
            alias_decision.acceptedExactOwner,
            sender_state_decision.valid,
            request_command_valid && response_command_valid &&
                response_port_matches && response_size_valid,
            ledger_result == LogicalStreamResponseResult::Accepted,
            response_counter_valid && continuation_counter_valid,
            response_credit_valid};
        if (!canMutateAcceptedResponse(response_preflight) &&
            decision.accepts()) {
            decision = {LogicalStreamResponseResult::Invalid,
                        TimingResponseDisposition::FatalOwnedCorruption, true,
                        sender_state_decision.releaseLogicalTop, true};
        }

        if (!decision.accepts()) {
            LogicalStreamResponseResult rejected = decision.result;
            if (rejected == LogicalStreamResponseResult::Accepted ||
                rejected == LogicalStreamResponseResult::Completed) {
                rejected = LogicalStreamResponseResult::Invalid;
            }
            LogicalStreamCounterDecision aborted_counter{};
            const bool counter_owned = has_authoritative_owner
                ? recorded_owner.counterOwned
                : request_command_valid &&
                      logicalStreamRequestKind(tmp.cmd) ==
                          LogicalStreamResponseKind::Write;
            const MemCmd &terminal_command = has_authoritative_owner
                ? recorded_owner.command : tmp.cmd;
            const Addr terminal_address = has_authoritative_owner
                ? recorded_owner.address : owned_address;
            const bool terminal_command_valid =
                terminal_command == MemCmd::ReadReq ||
                terminal_command == MemCmd::ReadExReq ||
                terminal_command == MemCmd::WriteReq;
            bool aborted_counter_valid = !counter_owned;
            if (stream != nullptr && terminal_command_valid &&
                counter_owned) {
                const LogicalStreamCounterEvent abort_event =
                    request_sent
                        ? LogicalStreamCounterEvent::ResponseAborted
                        : LogicalStreamCounterEvent::UnsentPacketAborted;
                aborted_counter = decideLogicalStreamCounterUpdate(
                    logicalStreamRequestKind(terminal_command), abort_event,
                    my_num_outstanding_stream_pkts[ledger_owner.maaID]);
                aborted_counter_valid = aborted_counter.valid;
            }
            if (stream != nullptr) {
                stream->rejectLogicalResponse(rejected);
                if (terminal_command == MemCmd::ReadReq ||
                    terminal_command == MemCmd::ReadExReq) {
                    stream->abortReadResponse(terminal_address);
                }
                stream->abortOwnedLogicalResponse(terminal_address);
            }
            erasePacketAliases(pkt);
            if (aborted_counter_valid && counter_owned) {
                my_num_outstanding_stream_pkts[ledger_owner.maaID] =
                    aborted_counter.value;
            }
            if (has_authoritative_owner) {
                releaseRecordedLogicalSenderState(pkt, recorded_owner);
                logical_state = nullptr;
                my_logical_packet_owners.erase(pkt);
            } else if (sender_state_decision.releaseLogicalTop) {
                releaseLogicalState(true);
            }
            DPRINTF(MAAPort,
                    "%s: exact logical packet at 0x%lx is corrupt (%d); "
                    "removed all production aliases before wrapper "
                    "destruction\n",
                    __func__, owned_address, static_cast<int>(rejected));
            return fatalOwnedDisposition();
        }

        erasePacketAliases(pkt);
        my_num_outstanding_stream_pkts[ledger_owner.maaID] =
            response_counter.value;
        if (has_authoritative_owner) {
            releaseRecordedLogicalSenderState(pkt, recorded_owner);
            logical_state = nullptr;
            my_logical_packet_owners.erase(pkt);
        } else {
            releaseLogicalState(true);
        }
        const LogicalStreamResponseResult accepted =
            tmp.cmd == MemCmd::WriteReq
                ? stream->writeResponseReceived(received_tag,
                                                sender_line_address)
                : stream->logicalResponseReceived(received_tag,
                                                  sender_line_address,
                                                  response_kind);
        bool delivery_valid =
            accepted == LogicalStreamResponseResult::Accepted ||
            accepted == LogicalStreamResponseResult::Completed;
        if (delivery_valid && tmp.cmd != MemCmd::WriteReq) {
            delivery_valid = stream->recvData(response_address,
                                              pkt->getPtr<uint8_t>());
        }
        if (!delivery_valid) {
            DPRINTF(MAAPort,
                    "%s: validated logical response at 0x%lx failed owner "
                    "delivery after complete ownership detachment\n",
                    __func__, owned_address);
            return fatalOwnedDisposition();
        }
        afterDeleteCompletion = {
            AfterDeleteCompletionKind::PromoteDeferred, -1, owned_address};
        return TimingResponseDisposition::Retired;
    }

    bool owners_valid =
        tmp.maaIDs.size() == tmp.funcUnits.size() && !tmp.maaIDs.empty();
    for (std::size_t index = 0;
         owners_valid && index < tmp.maaIDs.size(); ++index) {
        const int maa_id = tmp.maaIDs[index];
        const FuncUnitType unit = tmp.funcUnits[index];
        owners_valid = maa_id >= 0 &&
            ((unit == FuncUnitType::INDIRECT &&
              static_cast<unsigned>(maa_id) < num_indirect_units_total) ||
             (unit == FuncUnitType::STREAM &&
              static_cast<unsigned>(maa_id) < num_maas));
    }
    LogicalStreamResponseKind expected_kind =
        LogicalStreamResponseKind::Read;
    bool request_command_valid = true;
    if (tmp.cmd == MemCmd::ReadReq) {
        expected_kind = LogicalStreamResponseKind::Read;
    } else if (tmp.cmd == MemCmd::ReadExReq) {
        expected_kind = LogicalStreamResponseKind::ReadEx;
    } else if (tmp.cmd == MemCmd::WriteReq) {
        expected_kind = LogicalStreamResponseKind::Write;
    } else {
        request_command_valid = false;
    }
    const LogicalStreamResponseKind terminal_kind =
        tmp.virtualRetirement
        ? LogicalStreamResponseKind::Write
        : (request_command_valid
               ? expected_kind
               : (response_command_valid
                      ? response_kind
                      : LogicalStreamResponseKind::Read));
    const bool terminal_owner_identified = owners_valid &&
        (request_command_valid || response_command_valid ||
         tmp.virtualRetirement);
    const bool response_size_valid =
        hasExpectedLogicalStreamResponseSize(pkt->getSize(),
                                             tmp.expectedResponseBytes);
    const bool read_size_contract =
        expected_kind == LogicalStreamResponseKind::Write ||
        tmp.expectedResponseBytes == ExpectedLogicalStreamResponseBytes;
    const bool write_owners_valid =
        expected_kind != LogicalStreamResponseKind::Write ||
        (tmp.virtualRetirement && tmp.maaIDs.size() == 1 &&
         tmp.funcUnits.size() == 1 &&
         tmp.funcUnits[0] == FuncUnitType::INDIRECT);
    const NormalFatalOwnerDecision normal_fatal_settlement =
        decideNormalFatalOwnerSettlement(
            terminal_kind, tmp.sent, tmp.virtualRetirement);
    bool settlement_counters_valid = terminal_owner_identified;
    for (std::size_t index = 0;
         settlement_counters_valid &&
         normal_fatal_settlement.settleOutstandingCounter &&
         index < tmp.maaIDs.size(); ++index) {
        const uint32_t counter =
            tmp.funcUnits[index] == FuncUnitType::INDIRECT
            ? my_num_outstanding_indirect_pkts[tmp.maaIDs[index]]
            : my_num_outstanding_stream_pkts[tmp.maaIDs[index]];
        settlement_counters_valid = counter != 0;
    }
    const bool response_credit_valid = expected_response_port == nullptr ||
        expected_response_port->canSettleOwnedResponseCredit();
    bool unit_owners_valid = owners_valid;
    for (std::size_t index = 0;
         unit_owners_valid && index < tmp.maaIDs.size(); ++index) {
        const int maa_id = tmp.maaIDs[index];
        if (expected_kind == LogicalStreamResponseKind::Write) {
            unit_owners_valid = tmp.funcUnits[index] ==
                                    FuncUnitType::INDIRECT &&
                indirectAccessUnits[maa_id].canAbortRetirementWrite(
                    owned_address);
        } else if (tmp.funcUnits[index] == FuncUnitType::INDIRECT) {
            unit_owners_valid = indirectAccessUnits[maa_id]
                .hasReadResponseOwner(owned_address, tmp.cached);
        } else {
            unit_owners_valid = streamAccessUnits[maa_id]
                .hasReadResponseOwner(owned_address);
        }
    }
    const bool normal_valid =
        owners_valid && request_command_valid && response_command_valid &&
        response_size_valid && read_size_contract &&
        response_kind == expected_kind && tmp.sent &&
        response_port_matches && tmp.paddr == owned_address &&
        response_paddr == owned_address && response_address == owned_address &&
        alias_decision.acceptedExactOwner &&
        sender_state_decision.valid && write_owners_valid &&
        settlement_counters_valid && unit_owners_valid &&
        response_credit_valid &&
        afterDeleteCompletion.kind == AfterDeleteCompletionKind::None;
    if (!normal_valid) {
        if (terminal_owner_identified) {
            for (std::size_t index = 0; index < tmp.maaIDs.size(); ++index) {
                const int maa_id = tmp.maaIDs[index];
                if (normal_fatal_settlement.abortReadOwner) {
                    if (tmp.funcUnits[index] == FuncUnitType::INDIRECT) {
                        indirectAccessUnits[maa_id].abortReadResponse(
                            owned_address, tmp.cached);
                    } else {
                        streamAccessUnits[maa_id].abortReadResponse(
                            owned_address);
                    }
                }
                if (normal_fatal_settlement.abortRetirementWrite &&
                    tmp.funcUnits[index] == FuncUnitType::INDIRECT) {
                    indirectAccessUnits[maa_id].abortRetirementWrite(
                        owned_address);
                }
                if (normal_fatal_settlement.settleOutstandingCounter) {
                    uint32_t &counter =
                        tmp.funcUnits[index] == FuncUnitType::INDIRECT
                        ? my_num_outstanding_indirect_pkts[maa_id]
                        : my_num_outstanding_stream_pkts[maa_id];
                    if (counter != 0)
                        --counter;
                }
            }
        }
        erasePacketAliases(pkt);
        if (logical_state_release_safe)
            releaseLogicalState(true);
        DPRINTF(MAAPort,
                "%s: exact normal packet at 0x%lx is corrupt; removed all "
                "production aliases before wrapper destruction\n",
                __func__, owned_address);
        return fatalOwnedDisposition();
    }

    erasePacketAliases(pkt);
    if (response_kind == LogicalStreamResponseKind::Write) {
        const int maa_id = tmp.maaIDs[0];
        --my_num_outstanding_indirect_pkts[maa_id];
        afterDeleteCompletion = {
            AfterDeleteCompletionKind::RetirementWrite, maa_id,
            owned_address};
        return TimingResponseDisposition::Retired;
    }

    bool delivery_valid = true;
    for (std::size_t i = 0; i < tmp.maaIDs.size(); ++i) {
        if (tmp.funcUnits[i] == FuncUnitType::INDIRECT) {
            delivery_valid = indirectAccessUnits[tmp.maaIDs[i]].recvData(
                response_address, pkt->getPtr<uint8_t>(), tmp.cached);
        } else {
            delivery_valid = streamAccessUnits[tmp.maaIDs[i]].recvData(
                response_address, pkt->getPtr<uint8_t>());
        }
        if (!delivery_valid)
            break;
    }
    if (!delivery_valid) {
        DPRINTF(MAAPort,
                "%s: exact normal response at 0x%lx was rejected by its "
                "owner after complete ownership detachment\n",
                __func__, owned_address);
        return fatalOwnedDisposition();
    }
    afterDeleteCompletion = {
        AfterDeleteCompletionKind::PromoteDeferred, -1, owned_address};
    return TimingResponseDisposition::Retired;
}

void
MAA::completeTimingResponseAfterDelete(bool commitOwnerCompletion)
{
    const AfterDeleteCompletion completion = afterDeleteCompletion;
    afterDeleteCompletion = {};
    if (!commitOwnerCompletion ||
        completion.kind == AfterDeleteCompletionKind::None) {
        return;
    }
    if (completion.kind == AfterDeleteCompletionKind::PromoteDeferred) {
        sendNextDeferredPacket(completion.address);
        return;
    }
    panic_if(completion.kind != AfterDeleteCompletionKind::RetirementWrite ||
                 completion.maaID < 0 ||
                 static_cast<unsigned>(completion.maaID) >=
                     num_indirect_units_total,
             "%s: invalid post-delete response completion\n", __func__);
    indirectAccessUnits[completion.maaID].retirementWriteComplete(
        completion.address);
    sendNextDeferredPacket(completion.address);
}
void MAA::scheduleSendCacheEvent(int latency) {
    DPRINTF(MAAPort, "%s: scheduling send cache packet in the next %d cycles!\n", __func__, latency);
    panic_if(latency < 0, "Negative latency of %d!\n", latency);
    Tick new_when = getClockEdge(Cycles(latency));
    if (!sendCacheEvent.scheduled()) {
        schedule(sendCacheEvent, new_when);
    } else {
        Tick old_when = sendCacheEvent.when();
        DPRINTF(MAAPort, "%s: send cache packet already scheduled for tick %d\n", __func__, old_when);
        if (new_when < old_when) {
            DPRINTF(MAAPort, "%s: rescheduling for tick %d!\n", __func__, new_when);
            reschedule(sendCacheEvent, new_when);
        }
    }
}
void MAA::scheduleSendMemEvent(int latency) {
    DPRINTF(MAAPort, "%s: scheduling send mem packet in the next %d cycles!\n", __func__, latency);
    panic_if(latency < 0, "Negative latency of %d!\n", latency);
    Tick new_when = getClockEdge(Cycles(latency));
    if (!sendMemEvent.scheduled()) {
        schedule(sendMemEvent, new_when);
    } else {
        Tick old_when = sendMemEvent.when();
        DPRINTF(MAAPort, "%s: send mem packet already scheduled for tick %d\n", __func__, old_when);
        if (new_when < old_when) {
            DPRINTF(MAAPort, "%s: rescheduling for tick %d!\n", __func__, new_when);
            reschedule(sendMemEvent, new_when);
        }
    }
}
} // namespace gem5
