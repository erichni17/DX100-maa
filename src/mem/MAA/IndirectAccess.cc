...
void IndirectAccessUnit::allocate(int _my_indirect_id,
                                  ...
                                  bool _direct_index_force_cache,
    ...
    direct_index_force_cache = _direct_index_force_cache;
...
bool IndirectAccessUnit::receiveDirectIndex(Addr addr, uint8_t *dataptr,
    ...
    if (pending == direct_index_pending_lines.end())
        return false;
    if (is_block_cached)
        (*maa->stats.IND_VirtIndexCacheResponses[my_indirect_id])++;
    else
        (*maa->stats.IND_VirtIndexMemResponses[my_indirect_id])++;
