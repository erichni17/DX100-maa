...
MAA::MAAStats::MAAStats(statistics::Group *parent, int num_indirect_units, MAA *_maa)
    : statistics::Group(parent),
      ...
      ADD_STAT(
          rmwProfInstructions, statistics::units::Count::get(),
          "RMW instructions observed by the non-functional opportunity "
          "profiler"),
      ADD_STAT(
          rmwProfEligibleInstructions, statistics::units::Count::get(),
          "RMW instructions containing only eligible integer ADD/MIN/MAX "
          "updates"),
      ADD_STAT(
          rmwProfUpdates, statistics::units::Count::get(),
          "condition-enabled RMW updates observed by the opportunity "
          "profiler"),
      ADD_STAT(
      ...
      ADD_STAT(
          rmwProfUniqueLines, statistics::units::Count::get(),
          "sum of per-instruction unique eligible destination cache lines"),
      ADD_STAT(
          rmwProfBaselineReadExRequests, statistics::units::Count::get(),
          "ReadEx requests generated by the unmodified profiled RMW path"),
      ADD_STAT(
          rmwProfEligibleBaselineReadExRequests,
    ...
    rmwProfInstructions.flags(statistics::nozero);
    rmwProfEligibleInstructions.flags(statistics::nozero);
    rmwProfUpdates.flags(statistics::nozero);
    rmwProfEligibleUpdates.flags(statistics::nozero);
    rmwProfUniqueWords.flags(statistics::nozero);
    ...
    const std::array<std::string, 4> rmw_budget_names = {
        "4KiB", "8KiB", "16KiB", "32KiB"};
    for (const auto &budget : rmw_budget_names) {
        RMWProfLineHits.push_back(new statistics::Scalar(
            this, ("RMWProf" + budget + "LineHits").c_str(),
            statistics::units::Count::get(),
            "eligible line updates merged by a four-way shadow delta buffer"));
        RMWProfLineMisses.push_back(new statistics::Scalar(
            this, ("RMWProf" + budget + "LineMisses").c_str(),
            statistics::units::Count::get(),
            "line allocations (and eventual RMW transactions) in a four-way "
            "shadow delta buffer"));
        RMWProfEvictions.push_back(new statistics::Scalar(
            this, ("RMWProf" + budget + "Evictions").c_str(),
            statistics::units::Count::get(),
            "capacity/conflict drains in a four-way shadow delta buffer"));
        RMWProfAvoidableReadExRequests.push_back(new statistics::Scalar(
            this, ("RMWProf" + budget + "AvoidableReadExRequests").c_str(),
            statistics::units::Count::get(),
            "baseline ReadEx requests avoided by the shadow delta buffer; "
            "each also avoids one paired write"));
        RMWProfExtraReadExRequests.push_back(new statistics::Scalar(
            this, ("RMWProf" + budget + "ExtraReadExRequests").c_str(),
            statistics::units::Count::get(),
            "shadow delta-buffer ReadEx requests beyond the existing "
            "row-table baseline"));
        RMWProfLineHits.back()->flags(statistics::nozero);
        RMWProfLineMisses.back()->flags(statistics::nozero);
