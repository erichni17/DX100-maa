...
MAA::MAA(const MAAParams &p)
    ...
    panic_if(!maa::CompleteLineDrainBudget::validIssueWidth(
             ...
             "got %u\n",
             virtual_complete_line_drain_lines_per_cycle);
    panic_if(!maa::CompleteLinePayloadStaging::validWidth(
                 virtual_complete_line_payload_words_per_cycle),
             "Virtual complete-line payload width must be one of "
             "0/1/2/4/8, got %u\n",
             virtual_complete_line_payload_words_per_cycle);
...
MAA::MAAStats::MAAStats(statistics::Group *parent, int num_indirect_units, MAA *_maa)
    ...
    for (int indirect_id = 0; indirect_id < num_indirect_units; indirect_id++) {
        ...
        IND_VirtCompleteLinePayloadStageIssues.push_back(
            new statistics::Scalar(
                this,
                MAKE_INDIRECT_STAT_NAME(
                    "IND_VirtCompleteLinePayloadStageIssues"),
                statistics::units::Count::get(),
                "complete virtual-combiner lines entering bounded payload "
                "staging"));
        IND_VirtCompleteLinePayloadStageCompletions.push_back(
            new statistics::Scalar(
                this,
                MAKE_INDIRECT_STAT_NAME(
                    "IND_VirtCompleteLinePayloadStageCompletions"),
                statistics::units::Count::get(),
                "complete virtual-combiner lines completing bounded payload "
                "staging"));
        IND_VirtCompleteLinePayloadStageWaitCycles.push_back(
            new statistics::Scalar(
                this,
                MAKE_INDIRECT_STAT_NAME(
                    "IND_VirtCompleteLinePayloadStageWaitCycles"),
                statistics::units::Cycle::get(),
                "MAA cycles with a complete virtual-combiner line waiting "
                "for bounded payload staging"));
        IND_VirtCompleteLinePayloadStagePeakOccupancy.push_back(
            new statistics::Scalar(
                this,
                MAKE_INDIRECT_STAT_NAME(
                    "IND_VirtCompleteLinePayloadStagePeakOccupancy"),
                statistics::units::Count::get(),
                "sum of per-instruction peak complete-line payload staging "
                "metadata occupancy"));
