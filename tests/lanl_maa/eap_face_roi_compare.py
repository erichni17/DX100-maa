...
event = m5.simulate()
if event.getCause() != "workbegin" or event.getCode() != 0:
    raise RuntimeError(
        "EAP ROI program missed workbegin: "
        f"cause={event.getCause()} code={event.getCode()}"
    )

m5.stats.reset()
event = m5.simulate()
if event.getCause() != "workend" or event.getCode() != 0:
    raise RuntimeError(
        "EAP ROI program missed workend: "
        f"cause={event.getCause()} code={event.getCode()}"
    )
m5.stats.dump()
...
if event.getCode() != 0:
    raise RuntimeError(
        "EAP ROI post-marker oracle failed: "
        f"cause={event.getCause()} code={event.getCode()}"
    )
