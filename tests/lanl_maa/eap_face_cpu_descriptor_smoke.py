...
class MAACoherenceCache(L1Cache):
    # Selected only for the tested XRAGE/SPARTA-derived microbenchmark
    # envelope; EAP/FLAG validation is the purpose of this new path.
    pass
...
system.lanl_maa = LANLMAA(
    ...
    descriptor_slots=metadata.get("descriptor_slots", 1),
    max_descriptor_items=metadata["faces"],
    control_addr=metadata["control_paddr"],
    control_size=metadata["control_bytes"],
    operation_entries=metadata["faces"],
    continuation_entries=metadata["faces"],
    line_entries=32,
    update_entries=64,
    update_banks=8,
...
if event.getCode() != 0:
    raise RuntimeError(
        f"EAP face CPU descriptor program failed: cause={event.getCause()} "
