...
def test_api_duplicate_predicate_order_and_generation():
    ...
    assert "operation == 0 ? predicates[operation].data() : nullptr" in test
    ...
    main = test[test.index("int\nmain(") :]
    assert main.index("m5_checkpoint(0, 0)") < main.index(
        "m5_work_begin(0, 0)"
    )
    assert main.index("m5_work_begin(0, 0)") < main.index(
        "m5_reset_stats(0, 0)"
    )
    assert main.index("m5_reset_stats(0, 0)") < main.index("alloc_MAA()")
    assert main.index("alloc_MAA()") < main.index("initializeInputs(")
...
def test_four_arm_runner_is_matched_for_soa_physical_pair():
    ...
    assert "IND_SoaJitIndexLineResponses" in runner
    ...
    assert "$context_high_water -eq 2 && $context_stalls -gt 0" in runner
